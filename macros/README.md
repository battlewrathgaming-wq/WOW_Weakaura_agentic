...
macros/
  ...
  basis/                      FACT — sourced or proven. cite this.
    ...
    execution-model.md        how lines run · cast-gating · out-of-range NO fallthrough · modifiers · /click + stance-by-position (test+source)
    ui-errors.md              suppressing the red "cannot cast" text — the UI_ERRORS_OFF handler + the /run line + the AddMessage filter (sourced read)
