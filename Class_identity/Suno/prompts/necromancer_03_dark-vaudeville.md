# Prompt — Necromancer × Form 03 (Dark Vaudeville) · Driver B

_Form: [03 Dark Vaudeville (Gallows Whimsy)](../forms/03_dark-vaudeville.md). **Battlewrath's ask:
focus on the grand gothic organ** ("such a fun sound") — so this **bends 03 organ-forward** (the grand
pipe organ leading, rather than 03's default accordion/fiddle). Content vibe **(my proposal — the content
corner is yours to redraw):** the necromancer as a gleeful showman, the **"Maestro of the Dead,"**
conducting her army like a grand macabre performance — sinister-playful + gallows humor. Driver B, method
applied (line structure, forecast tags). **No banshee here** — that wail is desperate/funereal; it doesn't
fit the grin. A first draft to run and grade._

---

## The lever set

**Lyrics mode:** `Write` · **Vocal Gender:** `Female` (a theatrical dark-cabaret showman voice) ·
**Weirdness:** `~35` (a touch more life for the whimsy) · **Style Influence:** `~80` · **Duration:**
`Custom ~3:30` (cabaret-tight; or Auto) · **Title:** `The Maestro of the Dead`

**Styles** _(front-loaded — organ pulled near the front, since it's the focus):_
```
gothic dark cabaret, grand gothic pipe organ lead, macabre vaudeville waltz, sinister and playful, theatrical and wry, gallows humor, theatrical female showman vocal, oom-pah brass, stomping waltz rhythm, gang backing vocals, tolling bells, bass-rich, bouncing 3/4 lilt, grand and grinning
```

**Exclude styles** _(03's lighter OUT-list — no guitar / metal wall / modern-electronic; note 03 keeps
its brightness and tinkle, so those are NOT excluded here):_
```
electric guitar, distorted guitar, heavy metal, EDM, dubstep, synth-pop, autotune, modern pop, hip hop, lo-fi
```

## Lyrics (Write box) — a macabre waltz, with instrumental sections to let the whimsy instruments speak

_Battlewrath's note: **more instrumental sections.** Added `[Organ Solo]` (before the voice enters),
`[Instrumental Break]` (brass + bells trading over the organ), the `[Breakdown]` organ centrepiece, and an
`[Instrumental Outro]` — so the fun instruments carry passages, not just back the vocals._

```
[Intro]
(a grand gothic organ flourish, building — let it ring; a music-hall stomp and waltzing oom-pah join)

[Organ Solo]
(the grand pipe organ takes a full turn — theatrical, cascading, gleeful — before any voice enters)

[Verse]
(theatrical, wry, a showman's grin)
Come in, come in —
mind the bones on the floor.
The dead don't mind the dust anymore.
(a little sinister)
I raised them all myself, you know.
And oh — how they love a show.

[Chorus]
(grand, bouncing waltz, gang vocals joining)
So dance! dance to the organ's groan!
A one, a two, a rattling bone!
(sinister-playful)
The Maestro of the dead has come —
and the crypt has never been so much fun.

[Instrumental Break]
(the whimsy instruments speak: oom-pah brass and tolling bells trade the melody over the grand organ, stomps keeping the waltz)

[Verse]
(wry, confiding)
They never tire. they never complain.
No aches, no sleep, no need for a brain.
(grinning)
A tidy little army, mine to keep —
and the one thing they never do is rest in peace.

[Chorus]
(bigger — organ swelling, stomps, the whole room)
So dance! dance to the organ's groan!
A one, a two, a rattling bone!
The Maestro of the dead has come —
and the crypt has never been so much fun.

[Breakdown]
(a wild grand-organ solo — theatrical, mad, cascading, the centrepiece)
(spoken, delighted)
Ahh... listen to them go.

[Instrumental Outro]
(the waltz winding down — grand organ, then bells, then a final flourish and a stomp)
(sly, quiet, over the last notes)
Take a bow, my darlings.
...and back to the ground you go.
```

---

## What to confirm

- **The organ** — is the grand gothic pipe organ **front and center** and *fun*, the way you wanted it —
  and does putting it in the playful form work (haunted-carnival vs 01/02's grand-solemn use)?
- **Instrumental sections** — do the added `[Organ Solo]` / `[Instrumental Break]` / `[Instrumental Outro]`
  let the whimsy instruments *speak* (organ, oom-pah brass, bells getting their own turns)?
- **Register** — does it land **sinister-playful / gallows-humor** (grinning, not grim), a clear tonal
  flip from 01 and 02?
- **Line structure** — does the short-line comic timing carry the wry delivery?
- **The vibe** — the "Maestro" showman conceit: fun, or too much? Your corner — redraw the persona freely.

## Provenance & grade

Composed 2026-07-30 by the Suno manager — form 03 (organ-forward bend) × necromancer, Driver B. Content
(the Maestro conceit) is the Suno manager's proposal. Original lyrics.

**RUN 2026-08-01 — went well (Battlewrath):** the **musical / instrumental sections landed** (the "let the
instruments speak" additions — organ solo, instrumental break, breakdown, outro), and the **voice lines
delivered snappy / witty, reflecting the theme** — the whimsy register + line-structure comic timing
worked. ⇒ the method now spans **all three tonal registers**: grim-driving (01), funereal-desperate (02),
gallows-whimsy (03).
