...
| ★★★ **THE PIN IS A SLOT YOU WRITE TO — A NEW SET NEEDS NO RELEASE, AND `Clear` GENUINELY WIPES THE VIEW** | addons · §290 · *(measured live, `records/20260817_170747_470__chain.json` — 6 beacons, 6 set / 6 clear / 6 arrive, 955 rows, 193 s)*. ★ **The overwrite is instantaneous:** `set` follows `clear` inside one tick and the very next engine distance reads the NEW beacon — 4.02 → 63.96 yd with no lag, no stale pointer, no handover. Battlewrath: *"the pin only cares about being set."* ⚠ So a "release-then-set" chain is not a thing the client requires; `SetSuperTrackedPosition` alone re-points. ★★ **BUT THE TERMINAL RELEASE IS A REQUIREMENT, NOT MANNERS** — the marker never releases itself and nothing in the client's flow clears it, so a finished route left set points indefinitely at a spent target, silently, looking live. `GetTargetState` reads **0 at every clear**, which is the client confirming the VIEW is gone rather than only the state changing — the one thing that had never been proven. ★ Reference implementation already ships: `COA_Landmarks/beacon.lua` `Beacon.Clear()` (AC-27) |
| ★★ **`GetTargetState` across a live route: `2` tracking · `4` arrived · `0` cleared** | addons · §290, same run. ★ It narrates a whole route on its own, and the `4` is the 5.5 yd flip (row above) appearing in a moving-target run rather than a held-pin one. ★★★ **And the calibration pair SURVIVES THE TARGET MOVING: worst `\|sd − od\|` = 1.99e-05 over 955 rows across six pins** — the same figure previously only ever measured against ONE pin held for a whole capture, so `od` tracks whatever the pin currently is with no drift at the switch. ⚠ Worth knowing for any arrival threshold: every arrival landed at **4.02–4.75 yd against a 5.0 trigger**, never closer, because at 0.2 s and walking pace the last yard is crossed between samples. **A 5 yd trigger fires at ~4.0–4.8** — the sampling resolution showing up exactly where the closed form says it will |
...
| **What a DungeonRun beacon/route/run IS** | **addons** | `addons/planning/dungeonrun_model.md` — ★★★ **the heading**: the mission, **capture is the only spawn**, the two lanes, **a beacon is a THEATRE not a point**, the ratchet, and what is deliberately absent. ⚠ `ARCHIVE__dungeonrun_poc.md` is a 70,000-word ARCHIVE routed to by kind — **do not read it through**. Short version first, always |
