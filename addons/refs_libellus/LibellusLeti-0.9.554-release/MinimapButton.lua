...
local function OpenLanguage()
    if Mancer.Options and Mancer.Options.OpenLanguage then
...
local function ShowMenu(anchor)
    ...
    local menu = {
        ...
        {
            text = L["MINIMAP_OPEN_HUB"] or "Open Hub",
            notCheckable = true,
            func = function()
                OpenHub()
            end,
        ...
        {
            text = L["MINIMAP_DISPLAY"] or "Settings",
            notCheckable = true,
            func = function()
                OpenDisplay()
            end,
        ...
        {
            text = L["MINIMAP_LANGUAGE"] or "Language",
            notCheckable = true,
            func = function()
                OpenLanguage()
            end,
        ...
        {
            text = L["MINIMAP_HIDE"] or "Hide minimap button",
            notCheckable = true,
            func = function()
...
local function CreateButton()
    ...
    button:SetScript("OnEnter", function(self)
        ...
        GameTooltip:AddLine(L["MINIMAP_TIP_LEFT"] or "Left-click: Open Hub", 0.8, 0.8, 0.8)
        GameTooltip:AddLine(L["MINIMAP_TIP_RIGHT"] or "Right-click: Menu", 0.8, 0.8, 0.8)
        GameTooltip:AddLine(L["MINIMAP_TIP_DRAG"] or "Drag: Move icon", 0.8, 0.8, 0.8)
        GameTooltip:Show()
    end)
