...
Advisor.MINION_TYPES = {
    ...
    skeletal_mage = {
        label = "Skeletal Mage",
        namePatterns = { "Skeletal Mage", "Grave Mage" },
        talentPatterns = { "Raise: Skeletal Mage" },
        spellNames = { "Raise: Skeletal Mage" },
        -- Creature 50075 from Raise: Skeletal Mage (500331) summon effect.
        summonSpellIds = { [500331] = true },
        buffSpellIds = { [805020] = true },
        trackMode = "buff",
        trackUnits = true,
        lifeForceCost = 2,
        defaultMax = 1,
    ...
    decaying_colossus = {
        label = "Decaying Colossus",
        namePatterns = { "Decaying Colossus" },
        talentPatterns = { "Raise: Decaying Colossus" },
        spellNames = { "Raise: Decaying Colossus" },
        summonSpellIds = { [500989] = true },
        -- No unique player buff (Flesh Symbiosis is shared with Abom) — sheet uses
        -- nameplate/summon tracking + GetExpectedOwnedCount talent fallback.
        trackMode = "buff",
        lifeForceCost = 3,
        defaultMax = 1,
        requiredForAlert = false,
    ...
    abomination = {
        label = "Abomination",
        namePatterns = { "Abomination", "Flesh Giant", "Unholy Colossus", "Flesh Golem" },
        talentPatterns = {
            "Raise: Abomination", "Army of the Dead", "Raise: Unholy Colossus", "Raise: Flesh Golem",
        },
        spellNames = {
            "Raise: Abomination", "Raise: Unholy Colossus", "Raise: Flesh Golem",
        },
        summonSpellIds = { [42650] = true, [500335] = true, [803139] = true },
        buffSpellIds = { [680760] = true, [685010] = true, [805017] = true },
        -- Ascension Chomp ranks (Abom attack-speed frenzy).
        damageSpellIds = { [707283] = true, [707284] = true, [707740] = true, [573072] = true },
        trackMode = "buff",
        lifeForceCost = 3,
        defaultMax = 1,
        requiredForAlert = false,
...
-- Minion-owned ability names for Advisor spell lists / tooltips only.
-- MinionDps does NOT use these to force CLEU damage into buckets — attribution
-- follows combat-log source GUID/name (and summon registration) only.
Advisor.MINION_DAMAGE_SPELLS = {
    ["Putrid Claws"] = "ghoul",
    ["Command: Ghouls"] = "ghoul",
    ["Frozen Barbs"] = "crypt_fiend",
    ["Command: Banshee"] = "banshee",
    ["Command: Skeletal Mage"] = "skeletal_mage",
    ["Disease Cloud"] = "abomination",
    ["Chomp"] = "abomination",
    ["Command: Bone Smash"] = "decaying_colossus",
    ["Bonestorm"] = "bone_wraith",
    ["Bone Storm"] = "bone_wraith",
    ["Frost Wyrm Breath"] = "frost_wyrm",
}
...
function Advisor:IsLesserZombieSummon(destName, spellId, spellName)
    ...
    -- Only Harvest Plague (the summon cast) — never Zombie Plague DoT name alone.
    if spellName then
        local normalized = self:NormalizeSpellName(spellName)
        if normalized == "Harvest Plague" then
            return true
        end
    end
...
-- GUIDs proven ours via target/mouseover/tooltip/plate text. Survive seed wipes so
-- nameplate tokens can re-bind without needing ownership FontStrings every pass.
function Advisor:RememberOwnedGuid(guid)
    local key = NormalizeGuidKey(guid)
    if not key then
        return
    end
    self.ownedGuidProof = self.ownedGuidProof or {}
    self.ownedGuidProof[key] = (GetTime and GetTime()) or 0
end

function Advisor:ForgetOwnedGuid(guid)
    local key = NormalizeGuidKey(guid)
    if key and self.ownedGuidProof then
        self.ownedGuidProof[key] = nil
    end
end

function Advisor:HasOwnedGuidProof(guid)
    local key = NormalizeGuidKey(guid)
    if not key or not self.ownedGuidProof then
        return false
    end
    return self.ownedGuidProof[key] ~= nil
end

function Advisor:IsSheetBindableSummonSource(source)
    return IsTrustedSummonSource(source) or source == "owned_visible"
end
...
function Advisor:IsAscensionNameplateMinionCandidate(unit, name, minionId)
    -- Ascension: Exists / Friend / Enemy / Controlled / tip ownership often lie on plates
    -- until targeted, while UnitName + UnitGUID still work. Bind on Name+GUID alone.
    if type(unit) ~= "string" or not unit:find("^nameplate", 1) then
        return false
    end
    if not minionId then
        return false
    end
    -- Only hard reject obvious player plates.
...
-- How many of this minion type our auras / temporary trackers say we currently have.
-- Soft plate fill must never exceed this — otherwise nearby necros' pets enter the HUD.
function Advisor:GetExpectedOwnedCount(minionId)
    if not minionId then
        return 0
    end
    if minionId == "lesser_zombie" then
        -- Do not call GetActiveZombieCount here (it seeds again). Cap soft-fill by army talent.
        if not self:HasUnrelentingArmy() then
            return 0
        end
        return self:GetMinionScanCap("lesser_zombie") or 8
    end
    if self:UsesBuffTracking(minionId) then
        local counts = self:ScanAurasForMinions()
        local aura = (counts and counts[minionId]) or 0
        if aura > 0 then
            return aura
        end
        -- Decaying Colossus (and similar) may share Abom buffs — still soft-fill when talent known.
    ...
    if self:UsesTemporaryTracking(minionId) then
        local untilT = self.temporaryActive and self.temporaryActive[minionId]
        if untilT and (GetTime and GetTime() or 0) < untilT then
            return 1
        end
    end
    return 0
...
-- Raw on-screen count of a minion type (no ownership filter). Used to detect when
-- neighbours' pets are mixed in — soft fill must stay off in that case.
function Advisor:CountVisibleMinionNameplates(minionId)
    if not minionId then
        return 0
    end
    local count = 0
    ...
    local function consider(unit)
        ...
        local id = self:ClassifyByGuid(guid) or self:ClassifyMinionName(name)
        if id ~= minionId then
            return
        end
        if guid then
            local key = tostring(guid):lower()
            if seen[key] then
                return
            end
            seen[key] = true
...
-- Soft plate fill is only safe when every visible plate of this type can be ours
-- (visible count <= aura/expected). Extra Ghouls/Aboms on screen = other necros.
function Advisor:CanSoftFillMinion(minionId)
    local expected = self:GetExpectedOwnedCount(minionId)
    if not expected or expected < 1 then
        return false
    end
    local visible = self:CountVisibleMinionNameplates(minionId)
    return visible > 0 and visible <= expected
end

-- Sheet / seed binding: proven ownership, OR soft plate only when auras expect that type
-- and no neighbour extras of the same type are on screen.
-- Soft must NEVER RememberOwnedGuid — that poisoned IsOwnedByPlayer for every nearby necro pet.
function Advisor:CanBindSheetUnit(unit, name)
    if not unit then
        return false
    end
    if self:IsOwnedByPlayer(unit, name) then
        return true
    end
    name = name or (UnitName and UnitName(unit))
    local guid = UnitGUID and UnitGUID(unit)
    local minionId = self:ClassifyByGuid(guid) or self:ClassifyMinionName(name)
    if minionId and self:IsAscensionNameplateMinionCandidate(unit, name, minionId) then
        if self:CanSoftFillMinion(minionId) then
            return true
        end
    end
    return false
...
function Advisor:CleanGuardianName(name)
    if not name or name == "" then
        return nil
    end
...
function Advisor:IsOwnedNameMinionId(minionId)
    return type(minionId) == "string" and minionId:sub(1, #OWNED_NAME_PREFIX) == OWNED_NAME_PREFIX
end

-- Dynamic sheet/DPS key for owned guardians that are not in MINION_TYPES yet.
function Advisor:OwnedNameMinionId(name)
    local cleaned = self:CleanGuardianName(name)
    if not cleaned then
        return nil
    end
    return OWNED_NAME_PREFIX .. string.lower(cleaned), cleaned
end

function Advisor:OwnedNameLabel(minionId, fallbackName)
    if fallbackName and fallbackName ~= "" then
        return fallbackName
    end
    if self:IsOwnedNameMinionId(minionId) then
...
function Advisor:TryClassifyVisibleMinionUnit(unit)
    ...
    -- Nameplates: do not trust UnitIsEnemy (often wrong until targeted).
    local isPlate = type(unit) == "string" and unit:find("^nameplate", 1)
    if not isPlate and UnitIsEnemy and UnitIsEnemy("player", unit) then
        return nil
    end

    -- Hard ownership (tip / plate text / remembered GUID / CLEU).
    local owned = self:IsOwnedByPlayer(unit, name)

    if not minionId then
        -- Dynamic: own guardian with no typed match → name key (sheet + scans).
        if owned then
            local id, cleaned = self:OwnedNameMinionId(name)
            if id then
                return id, cleaned or name
            end
        end
        return nil
    end
    ...
    -- Soft nameplate: only when auras expect this type AND no extra neighbour plates.
    if self:IsAscensionNameplateMinionCandidate(unit, name, minionId)
        and self:CanSoftFillMinion(minionId) then
        return minionId, name
    end
...
function Advisor:SeedSummonsFromVisibleUnits(force)
    ...
    -- Sticky only CLEU/cast summons. Soft owned_visible must not survive as ownership proof.
    local stickyMinion = {}
    local freshProof = {}
    if self.activeSummons then
        for guid, info in pairs(self.activeSummons) do
            if info and info.minionId and self:IsTrustedSummonSource(info.source) then
                local key = NormalizeGuidKey(guid)
                if key then
                    stickyMinion[key] = info.minionId
                    freshProof[key] = now
                end
            end
        end
    end
    -- Drop soft-poisoned GUID proofs from earlier builds that treated every plate as ours.
    self.ownedGuidProof = freshProof
    ...
    local seenKeys = {}
    local ownedThisPass = {} -- minionId -> accepted count this pass (hard + soft)

    local function accept(unit, name, minionId, hardProof)
        local guid = UnitGUID and UnitGUID(unit)
        ...
        seenKeys[key] = true
        -- Only hard ownership / CLEU sticky may RememberOwnedGuid.
        if hardProof then
            self:RememberOwnedGuid(guid)
        end
        self:RecordSummon(guid, name, minionId, nil, "owned_visible")
        self:CacheGuidUnit(guid, unit)
        ownedThisPass[minionId] = (ownedThisPass[minionId] or 0) + 1
        return true
    end
    ...
    local function consider(unit)
        ...
        -- 1) Hard ownership (tip / plate text / pet token).
        if self:IsOwnedByPlayer(unit, name) then
            accept(unit, name, minionId, true)
            return
        end

        -- 2) Rebind CLEU sticky GUID.
        if key and stickyMinion[key] == minionId then
            accept(unit, name, minionId, true)
            return
        end

        -- 3) Soft fill: Ascension plates when auras expect this type, and every
        --    visible plate of that type can be ours (no neighbour extras).
        if self:IsAscensionNameplateMinionCandidate(unit, name, minionId)
            and self:CanSoftFillMinion(minionId) then
            local expected = self:GetExpectedOwnedCount(minionId)
            if (ownedThisPass[minionId] or 0) < expected then
                accept(unit, name, minionId, false)
            end
        end
    end
    ...
    for _, unit in ipairs({ "target", "mouseover", "focus" }) do
        if self:IsOwnedByPlayer(unit) then
            canPrune = true
            break
        end
    end
...
function Advisor:IsLifeForceTextEnabled()
    if MancerDB.showLifeForceText ~= true then
        return false
    end
    return Ascension.IsAnimationNecromancer()
end
...
function Advisor:ShouldRunAdvisor()
    return self:IsMinionAdvisorEnabled()
        or self:IsStanceAdvisorEnabled()
        or self:IsLifeForceTextEnabled()
end
...
function Advisor:MapTalentNameToMinionId(talentName)
    ...
    if lower:find("decaying colossus", 1, true) then
        return "decaying_colossus"
    end
    if lower:find("abomination", 1, true) or lower:find("unholy colossus", 1, true)
        or lower:find("flesh golem", 1, true) or lower:find("flesh giant", 1, true)
        or lower:find("colossus", 1, true) or lower:find("golem", 1, true) then
        return "abomination"
    end
...
function Advisor:GetRepresentativeUnitForMinion(minionId)
    ...
    if self.activeSummons then
        for guid, info in pairs(self.activeSummons) do
            -- CLEU and owned_visible (target/mouseover/nameplate proof) both bind the sheet.
            if info and info.minionId == minionId and self:IsSheetBindableSummonSource(info.source) then
                if info.expiresAt and info.expiresAt <= now then
                    -- expired temporary
                else
                    local unit = info.unit
                    if unit and UnitGUID and GuidsMatch(UnitGUID(unit), guid) and self:CanBindSheetUnit(unit) then
                        return unit, guid, info.name
                    end
                    unit = self:ResolveUnitTokenFromGuid(guid)
                    if unit and self:CanBindSheetUnit(unit) then
                        info.unit = unit
    ...
    -- Prefer ownership-proven plates, then Ascension soft nameplate candidates.
    local soft
    for i = 1, 40 do
        local unit = "nameplate" .. i
        local id, name = self:TryClassifyVisibleMinionUnit(unit)
        if id == minionId then
            ...
            if not soft then
                soft = { unit = unit, guid = guid, name = name }
            end
        end
    end
    if soft then
        return soft.unit, soft.guid, soft.name
    end
...
function Advisor:UnitOwnershipTextSaysMine(unit)
    ...
    if ok and tipSaysMine() then
        tip:Hide()
        return true
    end

    -- Ascension nameplates: SetUnit often fails while UnitExists is false; GUID hyperlink still works.
    local guid = UnitGUID and UnitGUID(unit)
    if guid then
        tip:ClearLines()
        local okLink = pcall(function()
            tip:SetHyperlink("unit:" .. tostring(guid))
        end)
        if okLink and tipSaysMine() then
            tip:Hide()
            return true
        end
    end
...
local function RegionTreeHasOwnershipText(frame, playerName, depth)
    ...
    -- Skip Libellus synthetic owner tags (pooled plates were proving "owned" for everyone).
    if frame.mancerSyntheticOwner then
        return false
    end
    ...
    if frame.GetChildren then
        ...
        for i = 1, #children do
            ...
            if child and not child.mancerSyntheticOwner and child ~= frame.mancerOwnerTag then
                if RegionTreeHasOwnershipText(child, playerName, (depth or 0) + 1) then
                    return true
                end
            end
        end
...
-- Collect FontString text on a plate (ownership can be split across lines).
local function CollectPlateFontText(frame, chunks, depth)
    ...
    if frame.GetRegions then
        local regions = { frame:GetRegions() }
        for i = 1, #regions do
            CollectPlateFontText(regions[i], chunks, (depth or 0) + 1)
        end
    end
    if frame.GetChildren then
        local children = { frame:GetChildren() }
        for i = 1, #children do
            local child = children[i]
            if child and child ~= frame.mancerOwnerTag and not child.mancerSyntheticOwner then
                CollectPlateFontText(child, chunks, (depth or 0) + 1)
            end
        end
    end
end

local function PlateTreeSaysMine(frame, playerName)
    if not frame or not playerName then
        return false
    end
    if RegionTreeHasOwnershipText(frame, playerName, 0) then
        return true
    end
    local chunks = {}
    CollectPlateFontText(frame, chunks, 0)
    if #chunks == 0 then
        return false
    end
    return OwnershipTextSaysMine(table.concat(chunks, " "), playerName)
end
...
function Advisor:NameplateOwnershipTextSaysMine(unit)
    ...
    if plate and PlateTreeSaysMine(plate, playerName) then
        return true
    end
    -- Prefer the Ascension pool UnitFrame (where healthBar + ownership subtitle live).
    local hud = Mancer.MinionHpHudModule or Mancer.MinionHpHud
    if hud and hud.GetPlateUnitFrame then
        local uf = hud:GetPlateUnitFrame(unit)
        if uf and uf ~= plate and PlateTreeSaysMine(uf, playerName) then
            return true
        end
    end
    ...
    if type(unit) == "string" and unit:find("^nameplate", 1) then
        local frame = _G[unit]
        if frame and PlateTreeSaysMine(frame, playerName) then
            return true
        end
        local alt = _G["NamePlate" .. (unit:match("(%d+)$") or "")]
        if alt and alt ~= frame and PlateTreeSaysMine(alt, playerName) then
            return true
        end
    end
    return false
...
function Advisor:IsOwnedByPlayer(unit, name)
    ...
    local guid = UnitGUID and UnitGUID(unit)
    if guid then
        -- Re-bind nameplates for GUIDs we already proved (target/mouseover/tip).
        if self:HasOwnedGuidProof(guid) then
            return true
        end
        local _, info = self:FindActiveSummon(guid)
        ...
        -- Soft seed this pass only — do not upgrade to sticky GUID proof.
        -- Reject soft when neighbour plates outnumber our expected count.
        if info and info.source == "owned_visible"
            and self:CanSoftFillMinion(info.minionId) then
            return true
        end
    end
...
function Advisor:IsPlayerDamageSpell(spellName, spellId)
    spellId = tonumber(spellId)
    -- Explicit player spell IDs (CLEU sometimes omits/renames the string).
    if spellId == 500267 then
        return true -- Expunge Blight
    end
...
-- Typed MINION_TYPES first; then any other owned guardian discovered live (own only).
function Advisor:GetMinionTypeRoster()
    ...
    for _, minionId in ipairs(self:GetMinionSheetOrder()) do
        ...
        if count and count > 0 then
            ...
            seenIds[minionId] = true
        end
    end

    -- Dynamic owned guardians (typed misses + future Raises) — CanBindSheetUnit = own only.
    local dyn = {}
    local function consider(unit)
        ...
        if not self:CanBindSheetUnit(unit) then
            return
        end
        local name = UnitName and UnitName(unit)
        if not name or name == "" then
        ...
        local label
        if not minionId then
            minionId, label = self:OwnedNameMinionId(name)
            if not minionId then
                return
            end
        else
        ...
        if seenIds[minionId] then
            return
        end
        local row = dyn[minionId]
        if not row then
            dyn[minionId] = {
                minionId = minionId,
                label = label or self:OwnedNameLabel(minionId, name),
                count = 1,
                unit = unit,
                guid = guid,
                name = name,
    ...
    -- Counts from snapshot for owned:* already noted elsewhere
    for minionId, count in pairs(counts or {}) do
        if not seenIds[minionId] and self:IsOwnedNameMinionId(minionId) and count and count > 0 then
            if not dyn[minionId] then
                local unit, guid, name = self:GetRepresentativeUnitForMinion(minionId)
                dyn[minionId] = {
                    minionId = minionId,
                    label = self:OwnedNameLabel(minionId, name),
                    count = count,
                    unit = unit,
                    guid = guid,
                    name = name,
                }
            elseif count > dyn[minionId].count then
                dyn[minionId].count = count
            end
        end
    end

    local dynList = {}
    for _, row in pairs(dyn) do
        dynList[#dynList + 1] = row
    end
    table.sort(dynList, function(a, b)
        return tostring(a.label) < tostring(b.label)
    end)
    for _, row in ipairs(dynList) do
        roster[#roster + 1] = row
        seenIds[row.minionId] = true
    end
...
-- Spell IDs live in Mancer.PROC_AURAS (Util.lua) — shared with MinionDps.
Advisor.PROC_AURAS = Mancer.PROC_AURAS
...
function Advisor:GetAdvisorDisplayKey(display)
    ...
    for _, icon in ipairs(display.animateIcons or display.icons or {}) do
        -- Do NOT include ticking remaining seconds — that re-applied icon textures
        -- every second and made the static (non-swipe) part of A-bar icons flicker.
        -- Timers refresh from slot.cdStart / aliveUntil in FloatingText pulse.
        local flag = icon.ready and "R" or (icon.active and "A" or "C")
        local spell = tostring(icon.spellId or icon.minionId or "?")
        local tex = tostring(icon.texture or "")
        local cdBucket = 0
        local cdStart = icon.cdStart or icon.spellStart or icon.start
        if cdStart and cdStart > 0 then
            cdBucket = math.floor(cdStart * 5 + 0.5)
        end
        local alive = (icon.active and ((icon.activeRemaining or 0) > 0.05)) and "1" or "0"
        table.insert(parts, string.format("%s:%s:%s:%d:%s", spell, flag, tex, cdBucket, alive))
    end
...
function Advisor:GetAdvisorDisplay()
    ...
    if self:IsLifeForceTextEnabled() then
...
function Advisor:PrintStanceDebug()
    local function out(msg)
        local text = "|cff7fd4ffLibellus Leti stance|r " .. tostring(msg)
