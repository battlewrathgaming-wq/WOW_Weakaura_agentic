...
-- Combat HUD under Ascension/game menus (Prestige, bags, character, …).
local HUD_STRATA = "LOW"
...
local ANIMATE_ICON_BASE_SIZE = 36
local ANIMATE_DEFAULT_SCALE = 1.25
...
local ZOMBIE_ICON_BASE_SIZE = 36
local ZOMBIE_DEFAULT_SCALE = 1.15
...
local PROC_ICON_BASE_SIZE = 36
local PROC_DEFAULT_SCALE = 1.15
...
-- Never ClearAllPoints on SecureActionButton frames or their parent (animateReady).
-- Ascension often still blocks ClearAllPoints after InCombatLockdown clears and
-- prints "LibellusLeti prevented … <unnamed>:ClearAllPoints()".
-- On 3.3.5, SetPoint with the same point name replaces the prior anchor.
local function SafeAnimateSetPoint(frame, ...)
...
-- Place Animate strip on UIParent using absolute offsets from MancerAnchor.
-- Never SetPoint the secure strip relative to MancerHandleA — Ascension then
-- treats the A-handle itself as protected (blocked SetPoint on MancerHandleA).
local function PlaceAnimateStripAtOffset(ft, ox, oy)
    if not ft or not ft.animateReady or not ft.anchor then
        return false
    end
    if not CanTouchAnimateStrip() then
        ft.pendingAnimateStripAnchor = true
        return false
    end
    if ft.animateReady:GetParent() ~= UIParent then
        ft.animateReady:SetParent(UIParent)
    end
    local ax, ay = ft.anchor:GetCenter()
    local ux, uy = UIParent:GetCenter()
    if not ax or not ux then
        ft.pendingAnimateStripAnchor = true
        return false
    end
    ox = ox or ANIMATE_DEFAULT_OFFSET.x
    oy = oy or ANIMATE_DEFAULT_OFFSET.y
    -- Same geometry as BOTTOM→handle TOP + 4: strip bottom above A-handle center.
    local sx = (ax + ox) - ux
    local sy = (ay + oy + (HANDLE_SIZE * 0.5) + 4) - uy
    if not SafeAnimateSetPoint(ft.animateReady, "BOTTOM", UIParent, "CENTER", sx, sy) then
        ft.pendingAnimateStripAnchor = true
        return false
    end
    ft._animateReadyAnchored = true
    ft.pendingAnimateStripAnchor = nil
    return true
end

-- HUD frames that are NOT in the secure Animate parent chain.
local function SafeHudSetPoint(frame, ...)
    if not frame then
        return false
    end
    frame:SetPoint(...)
...
local function SafeAnchorSetPoint(anchor, ...)
    return SafeHudSetPoint(anchor, ...)
end
...
function FloatingText:GetScaledRadius()
    if Mancer.ArcBars and Mancer.ArcBars.GetRadius then
        return Mancer.ArcBars:GetRadius()
    end
    local aspect = 1
    if Mancer.Util and Mancer.Util.GetAspectHudScale then
        aspect = Mancer.Util.GetAspectHudScale()
    end
    return (MancerDB.arcRadius or 65) * self:GetScale() * aspect
end
...
function FloatingText:GetBaseArcDistance()
    ...
    local baseRadius = (MancerDB.arcRadius or 65) * SCALE_HANDLE_RADIUS_MULT * self:GetScale() * aspect
...
-- Shared DB scale clamp for Animate / Zombie / Proc / Advisor text.
local function GetClampedDbScale(dbKey, default, minS, maxS)
    local scale = tonumber(MancerDB and MancerDB[dbKey])
...
local function AdjustClampedDbScale(dbKey, default, minS, maxS, delta)
    local next = GetClampedDbScale(dbKey, default, minS, maxS) + (delta or 0)
    next = math.max(minS, math.min(maxS, next))
    MancerDB[dbKey] = math.floor(next * 100 + 0.5) / 100
    return MancerDB[dbKey]
end

-- Offset-drag pieces (T / A / Z / P): one stop/update path, per-piece layout hooks.
local OFFSET_DRAG_SPECS = {
    advisor = {
        moving = "advisorMoving",
        lastX = "lastAdvisorDragX",
        lastY = "lastAdvisorDragY",
        offX = "advisorDragOffsetX",
        offY = "advisorDragOffsetY",
        handle = "advisorHandle",
        dbKey = "advisorTextOffset",
        after = function(ft)
            ft:ApplyAdvisorLayout()
        end,
        layoutWhileDrag = function(ft, x, y)
            SafeHudSetPoint(ft.advisorHandle, "CENTER", ft.anchor, "CENTER", x, y)
            ft:LayoutAdvisorTexts()
        end,
    },
    animate = {
        moving = "animateMoving",
        lastX = "lastAnimateDragX",
        lastY = "lastAnimateDragY",
        offX = "animateDragOffsetX",
        offY = "animateDragOffsetY",
        handle = "animateHandle",
        dbKey = "animateBarOffset",
        after = function(ft)
            ft:ApplyAdvisorLayout()
        end,
        layoutWhileDrag = function(ft, x, y)
            SafeHudSetPoint(ft.animateHandle, "CENTER", ft.anchor, "CENTER", x, y)
            -- Strip uses absolute UIParent coords (never relative to A-handle).
            PlaceAnimateStripAtOffset(ft, x, y)
        end,
    },
    zombie = {
        moving = "zombieMoving",
        lastX = "lastZombieDragX",
        lastY = "lastZombieDragY",
        offX = "zombieDragOffsetX",
        offY = "zombieDragOffsetY",
        handle = "zombieHandle",
        dbKey = "zombieCounterOffset",
        after = function(ft)
            ft:ApplyAdvisorLayout()
        end,
        layoutWhileDrag = function(ft, x, y)
            SafeHudSetPoint(ft.zombieHandle, "CENTER", ft.anchor, "CENTER", x, y)
            if ft.zombieCounter then
                SafeHudSetPoint(ft.zombieCounter, "BOTTOM", ft.zombieHandle, "TOP", 0, 4)
            end
        end,
    },
    proc = {
        ...
        lastX = "lastProcDragX",
        lastY = "lastProcDragY",
        offX = "procDragOffsetX",
        offY = "procDragOffsetY",
        handle = "procHandle",
        dbKey = "procBarOffset",
        after = function(ft)
            ft:ApplyAdvisorLayout()
        ...
        layoutWhileDrag = function(ft, x, y)
            SafeHudSetPoint(ft.procHandle, "CENTER", ft.anchor, "CENTER", x, y)
            if ft.procStrip then
                SafeHudSetPoint(ft.procStrip, "CENTER", ft.procHandle, "CENTER", 0, 0)
            end
        end,
    },
}

local function BeginOffsetDrag(ft, specKey)
    local spec = OFFSET_DRAG_SPECS[specKey]
    if not spec or not ft[spec.handle] then
        return
    end
    ft[spec.moving] = true
    ...
    local ax, ay = ft[spec.handle]:GetCenter()
    ft[spec.offX] = ax - cx
    ft[spec.offY] = ay - cy
end

local function StopOffsetDrag(ft, specKey)
    local spec = OFFSET_DRAG_SPECS[specKey]
    if not spec or not ft[spec.moving] then
        return
    end
    ft[spec.moving] = false
    local x, y = ft[spec.lastX], ft[spec.lastY]
    if x and y then
        MancerDB[spec.dbKey] = {
            x = math.floor(x + 0.5),
            y = math.floor(y + 0.5),
...
local function UpdateOffsetDragFromCursor(ft, specKey)
    local spec = OFFSET_DRAG_SPECS[specKey]
    if not spec then
        return
    end
    ...
    local cx, cy = GetCursorPosition()
    cx, cy = cx / uiScale, cy / uiScale
    local parentX, parentY = ft.anchor:GetCenter()
    local x = (cx + (ft[spec.offX] or 0)) - parentX
    local y = (cy + (ft[spec.offY] or 0)) - parentY
    ft[spec.lastX] = x
    ft[spec.lastY] = y
    if spec.layoutWhileDrag then
        spec.layoutWhileDrag(ft, x, y)
    end
end

-- Letter handles (T / A / Z / P) share the same chrome + drag wiring.
local function CreateLetterHandle(ft, opts)
    local handle = CreateFrame("Button", "MancerHandle" .. (opts.letter or "X"), ft.anchor)
    handle:SetSize(HANDLE_SIZE, HANDLE_SIZE)
    handle:EnableMouse(false)
    handle:Hide()
    handle:SetScript("OnMouseDown", function()
        if opts.canStart and opts.canStart(ft) then
            BeginOffsetDrag(ft, opts.key)
        end
    end)
    handle:SetScript("OnMouseUp", function()
        StopOffsetDrag(ft, opts.key)
    end)
    handle:EnableMouseWheel(true)
    handle:SetScript("OnMouseWheel", function(_, delta)
        if ft.moveMode and opts.onWheel then
            opts.onWheel(ft, delta)
        end
    end)

    local mark = handle:CreateFontString(nil, "OVERLAY")
    Mancer.Util.ApplyFont(mark, 14)
    mark:SetPoint("CENTER", handle, "CENTER", 0, 0)
    mark:SetText(opts.letter or "?")
    mark:SetTextColor(0, 0, 0, 1)

    local bg = handle:CreateTexture(nil, opts.bgLayer or "ARTWORK")
    bg:SetPoint("TOPLEFT", 1, -1)
    bg:SetPoint("BOTTOMRIGHT", -1, 1)
    local c = opts.color or { 1, 1, 1, 0.9 }
    SetSolidTexture(bg, c[1], c[2], c[3], c[4] or 0.9)
    if opts.bgDrawLayer and bg.SetDrawLayer then
        bg:SetDrawLayer(opts.bgDrawLayer, 0)
    end

    ft[opts.key .. "Handle"] = handle
    ft[opts.key .. "HandleMark"] = mark
    ft[opts.key .. "HandleBg"] = bg
    return handle
end
...
function FloatingText:ApplyHudStrata()
    if self.anchor then
        self.anchor:SetFrameStrata(HUD_STRATA)
        if not self.moveMode then
            self.anchor:SetFrameLevel(24)
        end
    end
    if Mancer.ArcBars and Mancer.ArcBars.frame then
        Mancer.ArcBars.frame:SetFrameStrata(HUD_STRATA)
        if not (Mancer.ArcBars.editMode) then
            Mancer.ArcBars.frame:SetFrameLevel(16)
        end
        if Mancer.ArcBars.unifiedBox then
            Mancer.ArcBars.unifiedBox:SetFrameStrata(HUD_STRATA)
        end
    end
    if Mancer.MinionHpHud then
        if Mancer.MinionHpHud.frame then
            Mancer.MinionHpHud.frame:SetFrameStrata(HUD_STRATA)
        end
        if Mancer.MinionHpHud.RaiseHandle and not Mancer.MinionHpHud.moveMode then
            Mancer.MinionHpHud:RaiseHandle()
        end
    end
end
...
function FloatingText:New()
    ...
    -- Help text: mousewheel scales handles except bars — yellow squares resize only.
    self.barHandle:EnableMouseWheel(false)
    self.barHandle:SetScript("OnMouseWheel", nil)
    ...
    self.helpBoxText:SetText(
        "Drag letter handles to move panels (T/A/Z/P/H/B).\n"
            ...
            .. "Yellow squares resize bars. α sliders set transparency."
    )
    ...
    CreateLetterHandle(self, {
        key = "advisor",
        letter = "T",
        color = { 0.35, 0.85, 1.0, 0.9 },
        canStart = function(ft)
            return ft.moveMode and not ft.moving and not ft.animateMoving
        end,
        onWheel = function(ft, delta)
            ft:AdjustAdvisorTextScale(delta > 0 and ADVISOR_WHEEL_STEP or -ADVISOR_WHEEL_STEP)
        end,
    })
    ...
    self.advisorHit:SetScript("OnMouseDown", function()
        if self.moveMode and not self.moving and not self.animateMoving and not self.barMoving then
            BeginOffsetDrag(self, "advisor")
        end
    end)
    self.advisorHit:SetScript("OnMouseUp", function()
        StopOffsetDrag(self, "advisor")
    end)

    -- Animate-ready strip. Named so taint reports aren't "<unnamed>".
    -- Parent = UIParent (NOT MancerAnchor): SecureActionButtons under the anchor make
    -- Ascension treat ClearAllPoints on the whole HUD tree as protected.
    -- Set strata/level BEFORE creating SecureActionButton children — Ascension then
    -- blocks SetFrameStrata/SetFrameLevel on this frame entirely.
    self.animateReady = CreateFrame("Frame", "MancerAnimateReady", UIParent)
    self.animateReady:SetFrameStrata(HUD_STRATA)
    self.animateReady:SetFrameLevel(26)
    ...
    for i = 1, ANIMATE_ICON_SLOTS do
        ...
        -- Named so future taint reports aren't "<unnamed>".
        local visual = CreateFrame("Frame", "MancerAnimateVisual" .. i, slotFrame)
    ...
    CreateLetterHandle(self, {
        key = "animate",
        letter = "A",
        color = { 0.95, 0.55, 0.25, 0.95 },
        bgLayer = "BACKGROUND",
        bgDrawLayer = "BACKGROUND",
        canStart = function(ft)
            return ft.moveMode and not ft.moving and not ft.advisorMoving
        end,
        onWheel = function(ft, delta)
            ft:AdjustAnimateIconScale(delta > 0 and 0.08 or -0.08)
        end,
    })

    -- Strip on UIParent with absolute coords from A-offset — never SetPoint to A-handle.
    if self.animateReady then
        local o = MancerDB and MancerDB.animateBarOffset
        if not PlaceAnimateStripAtOffset(
            self,
            o and o.x or ANIMATE_DEFAULT_OFFSET.x,
            o and o.y or ANIMATE_DEFAULT_OFFSET.y
        ) then
    ...
    self.zombieCounter:SetScript("OnMouseDown", function()
        if self.moveMode and not self.moving and not self.advisorMoving and not self.animateMoving then
            BeginOffsetDrag(self, "zombie")
        end
    end)
    self.zombieCounter:SetScript("OnMouseUp", function()
        StopOffsetDrag(self, "zombie")
    end)
    ...
    CreateLetterHandle(self, {
        key = "zombie",
        letter = "Z",
        color = { 0.45, 0.85, 0.4, 0.9 },
        canStart = function(ft)
            return ft.moveMode and not ft.moving and not ft.advisorMoving and not ft.animateMoving
        end,
        onWheel = function(ft, delta)
            ft:AdjustZombieIconScale(delta > 0 and 0.08 or -0.08)
        end,
    })
    ...
    CreateLetterHandle(self, {
        key = "proc",
        letter = "P",
        color = { 0.7, 0.4, 0.95, 0.9 },
        canStart = function(ft)
            return ft.moveMode and not ft.moving and not ft.advisorMoving
                and not ft.animateMoving and not ft.zombieMoving
        end,
        onWheel = function(ft, delta)
            ft:AdjustProcIconScale(delta > 0 and 0.08 or -0.08)
        end,
    })

    self:EnsureAlphaSliders()
...
function FloatingText:EnsureAnimateReadyAnchored()
    if not self.animateReady or not self.anchor then
        return false
    end
    local o = MancerDB and MancerDB.animateBarOffset
    return PlaceAnimateStripAtOffset(
        self,
        o and o.x or ANIMATE_DEFAULT_OFFSET.x,
        o and o.y or ANIMATE_DEFAULT_OFFSET.y
    )
...
local function ClampHudAlpha(a)
    a = tonumber(a)
    if not a then
        return 1
    end
...
function FloatingText:GetHudAlpha(dbKey)
    return ClampHudAlpha(MancerDB and MancerDB[dbKey])
end

function FloatingText:ApplyArcBarAlpha(alpha)
    alpha = ClampHudAlpha(alpha or (MancerDB and MancerDB.arcBarAlpha) or 1)
    local bars = Mancer.ArcBars
    if not bars then
        return
    end
    if bars.manaBar then
        bars.manaBar:SetAlpha(alpha)
    end
    if bars.healthBar then
        bars.healthBar:SetAlpha(alpha)
    end
    if bars.runicBar then
        bars.runicBar:SetAlpha(alpha)
    end
end

function FloatingText:ApplyAnimateBarAlpha(alpha)
    alpha = ClampHudAlpha(alpha or (MancerDB and MancerDB.animateBarAlpha) or 1)
    if self.animateReady then
        self.animateReady:SetAlpha(alpha)
    end
end

function FloatingText:ApplyZombieCounterAlpha(alpha)
    alpha = ClampHudAlpha(alpha or (MancerDB and MancerDB.zombieCounterAlpha) or 1)
    if self.zombieCounter then
        self.zombieCounter:SetAlpha(alpha)
    end
end

function FloatingText:ApplyProcBarAlpha(alpha)
    alpha = ClampHudAlpha(alpha or (MancerDB and MancerDB.procBarAlpha) or 1)
    if self.procStrip then
        self.procStrip:SetAlpha(alpha)
    end
end

function FloatingText:ApplyHudAlphas()
    self:ApplyArcBarAlpha()
    self:ApplyAnimateBarAlpha()
    self:ApplyZombieCounterAlpha()
    self:ApplyProcBarAlpha()
    if Mancer.MinionHpHud and Mancer.MinionHpHud.ApplyAlpha then
        Mancer.MinionHpHud:ApplyAlpha()
    end
    if self.alphaSliders then
        for _, slider in pairs(self.alphaSliders) do
            if slider.RefreshThumb then
                slider:RefreshThumb()
            end
        end
    end
...
function FloatingText:EnsureAlphaSliders()
    if not self.anchor or not Mancer.UI or not Mancer.UI.CreateAlphaSlider then
        return
    end
    self.alphaSliders = self.alphaSliders or {}

    if not self.alphaSliders.arc then
        self.alphaSliders.arc = Mancer.UI.CreateAlphaSlider(self.anchor, {
            name = "MancerAlphaArc",
            vertical = true,
            length = 88,
            dbKey = "arcBarAlpha",
            apply = function(a)
                if Mancer.FloatingText then
                    Mancer.FloatingText:ApplyArcBarAlpha(a)
                end
            end,
        })
    end
    if not self.alphaSliders.animate then
        self.alphaSliders.animate = Mancer.UI.CreateAlphaSlider(self.anchor, {
            name = "MancerAlphaAnimate",
            vertical = false,
            length = 96,
            dbKey = "animateBarAlpha",
            apply = function(a)
                if Mancer.FloatingText then
                    Mancer.FloatingText:ApplyAnimateBarAlpha(a)
                end
            end,
        })
    end
    if not self.alphaSliders.zombie then
        self.alphaSliders.zombie = Mancer.UI.CreateAlphaSlider(self.anchor, {
            name = "MancerAlphaZombie",
            vertical = true,
            length = 56,
            dbKey = "zombieCounterAlpha",
            apply = function(a)
                if Mancer.FloatingText then
                    Mancer.FloatingText:ApplyZombieCounterAlpha(a)
                end
            end,
        })
    end
    if not self.alphaSliders.proc then
        self.alphaSliders.proc = Mancer.UI.CreateAlphaSlider(self.anchor, {
            name = "MancerAlphaProc",
            vertical = false,
            length = 96,
            dbKey = "procBarAlpha",
            apply = function(a)
                if Mancer.FloatingText then
                    Mancer.FloatingText:ApplyProcBarAlpha(a)
                end
            end,
        })
    end

    self:LayoutAlphaSliders()
end

function FloatingText:LayoutAlphaSliders()
    local sliders = self.alphaSliders
    if not sliders then
        return
    end

    if sliders.arc then
        -- Prefer next to B so a tall/wide yellow box doesn't park α at the bottom of the screen.
        if self.barHandle then
            SafeHudSetPoint(sliders.arc, "LEFT", self.barHandle, "RIGHT", 8, 0)
        else
            local host = Mancer.ArcBars and (Mancer.ArcBars.unifiedBox or Mancer.ArcBars.healthBar or Mancer.ArcBars.frame)
            if host then
                SafeHudSetPoint(sliders.arc, "LEFT", host, "RIGHT", 6, 0)
            else
                SafeHudSetPoint(sliders.arc, "LEFT", self.anchor, "RIGHT", 8, 0)
            end
        end
    end

    if sliders.animate then
        -- Sit under the A-handle (not under the icon strip — that covered the handle).
        local host = self.animateHandle or self.animateReady
        if host then
            SafeHudSetPoint(sliders.animate, "TOP", host, "BOTTOM", 0, -4)
        end
    end

    if sliders.zombie and self.zombieCounter then
        SafeHudSetPoint(sliders.zombie, "LEFT", self.zombieCounter, "RIGHT", 4, 0)
    end

    if sliders.proc and self.procStrip then
        SafeHudSetPoint(sliders.proc, "TOP", self.procStrip, "BOTTOM", 0, -6)
    end

    -- Refresh after points are set so track width/height are valid.
    for _, slider in pairs(sliders) do
        if slider and slider:IsShown() and slider.RefreshThumb then
            slider:RefreshThumb()
        end
    end
end

function FloatingText:UpdateAlphaSliderVisibility()
    self:EnsureAlphaSliders()
    local sliders = self.alphaSliders
    if not sliders then
        return
    end
    local show = self.moveMode and true or false

    local function SetSlider(slider, enabled)
        if not slider then
            return
        end
        if show and enabled then
            slider:Show()
        else
    ...
    SetSlider(sliders.arc, MancerDB.showManaBar ~= false or MancerDB.showHealthBar ~= false or MancerDB.showRunicBar ~= false)
    SetSlider(sliders.animate, MancerDB.showAnimateBar ~= false)
    SetSlider(sliders.zombie, MancerDB.showZombieCounter ~= false)
    SetSlider(sliders.proc, MancerDB.showProcBar ~= false)
    self:LayoutAlphaSliders()
end

function FloatingText:ApplyAdvisorLayout()
    -- Animate strip uses absolute UIParent placement from A-offset (not relative to A).
    ...
    if self.advisorHandle then
        SafeHudSetPoint(self.advisorHandle, "CENTER", self.anchor, "CENTER", offset.x or 0, offset.y or 28)
    ...
    if self.animateHandle then
        ...
        self.animateHandle:SetFrameLevel(self.anchor:GetFrameLevel() + 45)
        PlaceAnimateStripAtOffset(
            self,
            animateOffset.x or ANIMATE_DEFAULT_OFFSET.x,
            animateOffset.y or ANIMATE_DEFAULT_OFFSET.y
        )
    ...
    if self.advisorIcons then
        SafeHudSetPoint(self.advisorIcons, "CENTER", self.advisorHandle or self.anchor, "CENTER", 0, 0)
    ...
    if self.zombieCounter then
        ...
        SafeHudSetPoint(self.zombieCounter, "BOTTOM", self.zombieHandle or self.anchor, "TOP", 0, 4)
    ...
    if self.procStrip then
        SafeHudSetPoint(self.procStrip, "CENTER", self.procHandle or self.anchor, "CENTER", 0, 0)
...
function FloatingText:LayoutAdvisorTexts()
    ...
    if self.advisorText then
        if hasStance and hasLf then
            SafeHudSetPoint(self.advisorText, "BOTTOM", self.advisorHandle or self.anchor, "CENTER", 0, 4)
        else
            SafeHudSetPoint(self.advisorText, "CENTER", self.advisorHandle or self.anchor, "CENTER", 0, 0)
        end
    end

    if self.advisorLfText then
        if hasStance and hasLf then
            SafeHudSetPoint(self.advisorLfText, "TOP", self.advisorText, "BOTTOM", 0, -2)
        else
            SafeHudSetPoint(self.advisorLfText, "CENTER", self.advisorHandle or self.anchor, "CENTER", 0, 0)
        end
    end
...
function FloatingText:LayoutAdvisorHit()
    ...
    SafeHudSetPoint(self.advisorHit, "BOTTOM", self.advisorHandle, "BOTTOM", 0, -padY)
...
function FloatingText:GetAnimateIconScale()
    return GetClampedDbScale("animateIconScale", ANIMATE_DEFAULT_SCALE, MIN_ANIMATE_SCALE, MAX_ANIMATE_SCALE)
end

function FloatingText:GetAdvisorTextScale()
    return GetClampedDbScale("advisorTextScale", ADVISOR_DEFAULT_SCALE, MIN_ADVISOR_SCALE, MAX_ADVISOR_SCALE)
end
...
function FloatingText:AdjustAdvisorTextScale(delta)
    AdjustClampedDbScale(
        "advisorTextScale",
        ADVISOR_DEFAULT_SCALE,
        MIN_ADVISOR_SCALE,
        MAX_ADVISOR_SCALE,
        delta
    )
...
function FloatingText:GetZombieIconScale()
    return GetClampedDbScale("zombieIconScale", ZOMBIE_DEFAULT_SCALE, MIN_ZOMBIE_SCALE, MAX_ZOMBIE_SCALE)
end
...
function FloatingText:LayoutZombieCounter()
    ...
    if self.zombieIcon then
        SafeHudSetPoint(self.zombieIcon, "TOPLEFT", self.zombieCounter, "TOPLEFT", 0, 0)
        SafeHudSetPoint(self.zombieIcon, "BOTTOMRIGHT", self.zombieCounter, "BOTTOMRIGHT", 0, 0)
...
function FloatingText:GetProcIconScale()
    return GetClampedDbScale("procIconScale", PROC_DEFAULT_SCALE, MIN_PROC_SCALE, MAX_PROC_SCALE)
end
...
function FloatingText:AdjustProcIconScale(delta)
    AdjustClampedDbScale("procIconScale", PROC_DEFAULT_SCALE, MIN_PROC_SCALE, MAX_PROC_SCALE, delta)
    ...
    for i, slot in ipairs(self.animateIconSlots) do
        if i <= iconCount then
            if CanTouchAnimateButton(slot.frame) then
                ...
                -- No ClearAllPoints on SecureActionButton — SetPoint replaces CENTER.
                SafeAnimateSetPoint(
                    slot.frame,
                    "CENTER",
                    self.animateReady,
...
function FloatingText:ApplyAnimateIconState(slot, iconData)
    ...
    local texture = iconData.texture
    -- Only retouch the icon when the path changes — SetTexture(nil)/reset every
    -- refresh made the static icon flicker under a stable cooldown swipe.
    if texture and texture ~= slot._lastTexture then
        slot.icon:SetTexCoord(0.08, 0.92, 0.08, 0.92)
        slot.icon:SetTexture(texture)
        slot._lastTexture = texture
    elseif not texture and not slot._lastTexture then
        slot.icon:SetTexCoord(0.08, 0.92, 0.08, 0.92)
        slot.icon:SetTexture("Interface\\Icons\\Spell_Shadow_AnimateDead")
        slot._lastTexture = "Interface\\Icons\\Spell_Shadow_AnimateDead"
    end
    if slot.icon and not slot.icon:GetTexture() then
        slot.icon:SetTexture("Interface\\Icons\\Spell_Shadow_AnimateDead")
        slot._lastTexture = "Interface\\Icons\\Spell_Shadow_AnimateDead"
    end
    slot.icon:Show()
    ...
    if ready then
        ...
        slot.cdDuration = nil
        slot.aliveUntil = nil
        slot._lastCdStart = nil
        slot._lastCdDuration = nil
        slot.icon:SetVertexColor(1, 1, 1, 1)
        if slot.icon.SetDesaturated then
            slot.icon:SetDesaturated(false)
        end
    ...
    if slot.cooldown and cdStart and cdDuration and cdDuration > 0 and CooldownFrame_SetTimer then
        -- Re-arm swipe only when the CD window changes (not every advisor tick).
        if slot._lastCdStart ~= cdStart or slot._lastCdDuration ~= cdDuration then
            ArmAnimateCooldownNoCount(slot.cooldown)
            CooldownFrame_SetTimer(slot.cooldown, cdStart, cdDuration, 1)
            slot.cooldown:Show()
            slot._lastCdStart = cdStart
            slot._lastCdDuration = cdDuration
        elseif not slot.cooldown:IsShown() then
            slot.cooldown:Show()
        end
    elseif slot.cooldown then
        slot.cooldown:Hide()
...
function FloatingText:GetBarArcMidpoint()
    local radius = self:GetScaledRadius()
    if Mancer.ArcBars and Mancer.ArcBars.GetRadius then
...
function FloatingText:ApplyBarHandleLayout()
    ...
    local ox, oy = self:GetBarTransformOffset()
    SafeHudSetPoint(self.barHandle, "CENTER", self.anchor, "CENTER", midX + ox, midY + oy)
...
function FloatingText:UpdateBarPositionFromCursor()
    ...
    if self.barHandle then
        SafeHudSetPoint(self.barHandle, "CENTER", self.anchor, "CENTER", x, y)
    end
    if self.moveMode then
        self:ShowArcPreview()
    end
...
function FloatingText:UpdatePositionFromCursor()
    ...
    -- Animate strip is on UIParent — refresh absolute placement so it follows HUD drag.
    local o = MancerDB and MancerDB.animateBarOffset
    PlaceAnimateStripAtOffset(
        self,
        o and o.x or ANIMATE_DEFAULT_OFFSET.x,
...
function FloatingText:RefreshScaledLayout()
    ...
    if Mancer.ArcBars then
        ...
        if Mancer.ArcBars.editMode then
            ...
            if Mancer.ArcBars.runicBar then
                Mancer.ArcBars:ApplyBarVisual(Mancer.ArcBars.runicBar, MancerDB.runicColor or { 0.45, 0.85, 1.0 })
            end
        end
    end
...
-- Welcome wizard: only show move/scale handles for the active section.
-- focus: nil | "bars" | "ticks" | "combat" | "minions"
function FloatingText:SetWizardFocus(focus)
    self.wizardFocus = focus
    if focus then
        self.wizardOwnsMoveMode = true
        if not self.moveMode then
            self:SetMoveMode(true)
        else
            self:ApplyWizardFocusHandles()
        end
    else
        if self.wizardOwnsMoveMode then
            self.wizardOwnsMoveMode = false
            if self.moveMode then
                self:SetMoveMode(false)
            end
        elseif self.moveMode then
            self:ApplyWizardFocusHandles()
        end
    end
end

function FloatingText:ApplyWizardFocusHandles()
    local focus = self.wizardFocus
    if not focus or not self.moveMode then
        return
    end

    local db = MancerDB or {}
    local necro = db.necromancer or {}
    local bars = focus == "bars"
    local ticks = focus == "ticks"
    local combat = focus == "combat"
    local minions = focus == "minions"

    local wantBars = bars and (db.showManaBar or db.showHealthBar or db.showRunicBar)
    local wantAnimate = combat and db.showAnimateBar
    local wantZombie = combat and db.showZombieCounter
    local wantProc = combat and db.showProcBar
    local wantStance = combat and necro.stanceEnabled
    local wantLf = combat and db.showLifeForceText
    local wantAdvisorMove = wantStance or wantLf
    local wantTicks = ticks and (db.showMana or db.showHealth or db.showRegenRate)
    local wantMinions = minions and db.showMinionHpList
    ...
    if self.barHandle then
        if wantBars then
            self.barHandle:Show()
            self.barHandle:EnableMouse(true)
            self:ApplyBarHandleLayout()
        else
            self.barHandle:Hide()
            self.barHandle:EnableMouse(false)
    ...
    if self.advisorHit then
        if wantAdvisorMove then
            ...
            self.advisorHit:Hide()
            self.advisorHit:EnableMouse(false)
            self.advisorHit:EnableMouseWheel(false)
    ...
    if self.animateHandle then
        if wantAnimate then
            ...
            self.animateHandle:EnableMouse(true)
            self.animateHandle:EnableMouseWheel(true)
        else
            self.animateHandle:Hide()
            self.animateHandle:EnableMouse(false)
            self.animateHandle:EnableMouseWheel(false)
        end
    end
    if self.animateReady and CanTouchAnimateStrip() then
        self.animateReady:EnableMouse(wantAnimate and true or false)
        self.animateReady:EnableMouseWheel(wantAnimate and true or false)
    end

    if self.zombieHandle then
        if wantZombie then
            self.zombieHandle:Show()
            self.zombieHandle:EnableMouse(true)
            self.zombieHandle:EnableMouseWheel(true)
        else
            self.zombieHandle:Hide()
            self.zombieHandle:EnableMouse(false)
            self.zombieHandle:EnableMouseWheel(false)
        end
    end
    if self.zombieCounter then
        self.zombieCounter:EnableMouse(wantZombie and true or false)
        self.zombieCounter:EnableMouseWheel(wantZombie and true or false)
    end

    if self.procHandle then
        if wantProc then
            self.procHandle:Show()
            self.procHandle:EnableMouse(true)
            self.procHandle:EnableMouseWheel(true)
        else
            self.procHandle:Hide()
            self.procHandle:EnableMouse(false)
            self.procHandle:EnableMouseWheel(false)
        end
    end
    if self.procStrip then
        self.procStrip:EnableMouse(wantProc and true or false)
        self.procStrip:EnableMouseWheel(wantProc and true or false)
    end
    ...
    if Mancer.ArcBars then
        Mancer.ArcBars:SetEditMode(wantBars and true or false)
    end
    if Mancer.MinionHpHud and Mancer.MinionHpHud.SetMoveMode then
        Mancer.MinionHpHud:SetMoveMode(wantMinions and true or false)
    end

    if wantBars or wantAnimate or wantZombie or wantProc or wantAdvisorMove then
        self:ApplyAdvisorLayout()
        if wantZombie then
            self:UpdateZombieCounter()
        end
...
function FloatingText:SetMoveMode(enabled)
    ...
    local minionMove = enabled and (not self.wizardFocus or self.wizardFocus == "minions")
    if Mancer.MinionHpHud and Mancer.MinionHpHud.SetMoveMode then
        Mancer.MinionHpHud:SetMoveMode(minionMove)
    end
    ...
    if Mancer.ArcBars then
        ...
        if self.wizardFocus and enabled then
            self:ApplyWizardFocusHandles()
        else
...
function FloatingText:ShowArcPreview()
    ...
    for _, guide in ipairs(self.arcGuides) do
        ...
        SafeHudSetPoint(guide.mana, "CENTER", self.anchor, "CENTER", mx, my)
        guide.mana:Show()

        SafeHudSetPoint(guide.health, "CENTER", self.anchor, "CENTER", hx, hy)
        guide.health:Show()
    end
end
...
function FloatingText:ApplyConfig()
    ...
    SafeAnchorSetPoint(self.anchor, "CENTER", UIParent, "CENTER", cfg.anchorX, cfg.anchorY)
    self:ApplyHelpBoxLayout()
...
function FloatingText:ShowTick(text, color, side)
    ...
    -- SetPoint-only: ClearAllPoints on MancerAnchor children is blocked while Animate is secure.
...
local MAX_BAR_SCALE = 6.0
-- Was 120 — felt stuck mid-screen (arcs sit high; a tall edit box still
-- covers the character at max-down). Match other HUD element freedom.
...
local function ApplyVerticalFill(background, foreground, bar, texturePath, progress, color)
    ...
    -- Never ClearAllPoints/SetAllPoints — Ascension can attribute those to LibellusLeti
    -- as "<unnamed>:ClearAllPoints()" when anything in the HUD tree is secure-linked.
    -- Always use the same two-point + height scheme so points replace cleanly.
    if progress >= 0.999 then
        foreground:Show()
        foreground:SetPoint("BOTTOMLEFT", bar, "BOTTOMLEFT")
        foreground:SetPoint("BOTTOMRIGHT", bar, "BOTTOMRIGHT")
        foreground:SetHeight(barHeight)
    ...
    if progress <= 0.001 then
        ...
        background:SetHeight(barHeight)
...
function ArcBars:AdjustBarScale(delta)
    ...
    if self.runicBar then
        self:ApplyBarVisual(self.runicBar, self.runicBar.color or MancerDB.runicColor)
    end
...
function ArcBars:UpdateUnifiedEditorBounds()
    ...
    -- SetPoint-only (two corners replace prior TOPLEFT/BOTTOMRIGHT).
    ...
    if Mancer.FloatingText and Mancer.FloatingText.LayoutAlphaSliders then
        Mancer.FloatingText:LayoutAlphaSliders()
    end
end
...
function ArcBars:SetEditMode(enabled)
    ...
    -- Match FloatingText HUD strata so arcs stay under game menus.
    self.frame:SetFrameStrata(HUD_STRATA)
    ...
    if self.unifiedBox then
        ...
        self.unifiedBox:SetFrameStrata(HUD_STRATA)
    ...
    if enabled then
        ...
        if self.runicBar then
            self.runicBar.targetPercent = 0.75
            self.runicBar.displayPercent = 0.75
            self.runicBar:Show()
        end
        self:Layout()
        ...
        if self.runicBar then
            self:ApplyBarVisual(self.runicBar, MancerDB.runicColor or { 0.45, 0.85, 1.0 })
        end
        if Mancer.FloatingText and Mancer.FloatingText.ApplyBarHandleLayout then
            Mancer.FloatingText:ApplyBarHandleLayout()
        end
...
function ArcBars:ApplyBarTextures(path)
    ...
    for _, bar in ipairs({ self.manaBar, self.healthBar, self.runicBar }) do
        if bar then
...
function ArcBars:New()
    ...
    self.runicBar = CreateProgressBar(self.frame)
    self.runicBar.side = "mana"
    self.runicBar:SetFrameLevel((self.manaBar:GetFrameLevel() or 1) + 1)
...
function ArcBars:LayoutBar(bar, side)
    ...
    -- SetPoint-only: bars live under MancerAnchor (Animate secure strip).
...
-- Same arch as mana, shifted left (outward) so it sits beside it.
local RUNIC_LEFT_SHIFT = 0.22

function ArcBars:LayoutRunicBar()
    local bar = self.runicBar
    if not bar then
        return
    end
    self:LayoutBar(bar, "mana")

    local radius = self:GetRadius()
    local cx, cy, _, width = Mancer.Util.GetArcBounds("mana", radius)
    local t = self:GetTransform("unified")
    local manaW = math.max(20, math.floor(width * (t.width or 1) + 0.5))
    local shiftX = -math.floor(manaW * RUNIC_LEFT_SHIFT + 0.5)

    bar:SetPoint("CENTER", self.frame, "CENTER", cx + (t.offsetX or 0) + shiftX, cy + (t.offsetY or 0))
end

function ArcBars:Layout()
    if Mancer.FloatingText and Mancer.FloatingText.anchor then
        -- SetPoint-only: ClearAllPoints on anything tied to MancerAnchor taints on Ascension.
...
function ArcBars:UpdateValues()
    ...
    local runic, maxRunic = Mancer.Util.GetPlayerRunicPower()
    ...
    if self.runicBar then
        self:UpdateBar(
            ...
            runic,
            maxRunic,
            cfg.runicColor or { 0.45, 0.85, 1.0 },
            cfg.showRunicBar ~= false and Mancer.Util.HasRunicPowerBar()
        )
    end
end
...
function ArcBars:ApplyConfig()
    ...
    if Mancer.FloatingText and Mancer.FloatingText.ApplyArcBarAlpha then
        Mancer.FloatingText:ApplyArcBarAlpha()
    end
