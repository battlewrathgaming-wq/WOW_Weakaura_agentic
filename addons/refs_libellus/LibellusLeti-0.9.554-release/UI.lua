...
-- opts (optional): corner, edge, padL, padR, padT, padB — thinner/tighter for non-portrait frames.
function UI.ApplyMetalPortraitBorder(frame, opts)
...
-- Slimmer flush metal border for square dialogs (welcome wizard, etc.).
function UI.ApplySlimMetalBorder(frame)
    return UI.ApplyMetalPortraitBorder(frame, {
        corner = 22,
        topCorner = 22,
        edge = 18,
        padL = -2,
...
-- Movable Display-style chrome window (metal portrait + optional DISPLAY_BG art).
-- Returns window, contentPanel.
function UI.CreateMovableChromeWindow(name, opts)
    opts = opts or {}
    local parent = opts.parent or UIParent
    local frame
    if UI.CreateHubRootFrame then
        frame = UI.CreateHubRootFrame(name, parent)
    else
        frame = CreateFrame("Frame", name, parent)
    end

    frame:SetSize(opts.width or 580, opts.height or 640)
    frame:SetPoint(opts.point or "CENTER")
    ...
    if UI.SkinFrame then
        local bg = opts.bg or UI.DISPLAY_BG or {}
        UI.SkinFrame(frame, {
            nativeChrome = true,
            artAtlas = bg.atlas,
            artPath = bg.path,
    ...
    elseif UI.ApplyMetalPortraitBorder then
        UI.ApplyMetalPortraitBorder(frame)
    ...
    elseif frame.SetBackdrop then
        ...
        frame:SetBackdropBorderColor(0.18, 0.22, 0.24, 1)
        if UI.CreateNativeCloseButton then
            UI.CreateNativeCloseButton(frame)
        end
    end
    ...
    panel:SetPoint("TOPLEFT", opts.insetLeft or 22, opts.insetTop or -68)
    panel:SetPoint("BOTTOMRIGHT", -(opts.insetRight or 22), opts.insetBottom or 28)

    if opts.specialFrame ~= false and UISpecialFrames then
...
-- HUD move-mode transparency slider (vertical or horizontal).
-- Anchors to a host frame so it rides along when that section is dragged.
-- opts: vertical, length, thick, dbKey, apply (fn), minAlpha, label
function UI.CreateAlphaSlider(parent, opts)
    opts = opts or {}
    local vertical = opts.vertical and true or false
    local length = opts.length or 72
    local thick = opts.thick or 8
    ...
    if vertical then
        frame:SetSize(thick + pad * 2, length + pad * 2)
    else
        frame:SetSize(length + pad * 2, thick + pad * 2)
    end
    -- Stay on the parent strata so letter handles (A/Z/…) remain clickable above.
    if parent and parent.GetFrameStrata then
        frame:SetFrameStrata(parent:GetFrameStrata() or "MEDIUM")
    end
    ...
    local bg = Solid(frame, "BACKGROUND")
    bg:SetAllPoints()
    bg:SetVertexColor(0.05, 0.05, 0.08, 0.72)

    local track = Solid(frame, "ARTWORK")
    if vertical then
        track:SetPoint("TOP", frame, "TOP", 0, -pad)
        track:SetPoint("BOTTOM", frame, "BOTTOM", 0, pad)
        track:SetWidth(thick)
    else
        track:SetPoint("LEFT", frame, "LEFT", pad, 0)
        track:SetPoint("RIGHT", frame, "RIGHT", -pad, 0)
        track:SetPoint("CENTER", frame, "CENTER", 0, 0)
        track:SetHeight(thick)
    end
    track:SetVertexColor(0.18, 0.2, 0.22, 1)
    frame.track = track

    local fill = Solid(frame, "OVERLAY")
    fill:SetVertexColor(0.25, 0.95, 0.75, 0.85)
    frame.fill = fill

    local thumb = CreateFrame("Button", nil, frame)
    thumb:SetSize(thick + 6, thick + 6)
    thumb:EnableMouse(true)
    thumb:RegisterForClicks("LeftButtonUp", "LeftButtonDown")
    local thumbBg = Solid(thumb, "ARTWORK")
    thumbBg:SetAllPoints()
    thumbBg:SetVertexColor(1, 0.88, 0.25, 1)
    frame.thumb = thumb

    local label = frame:CreateFontString(nil, "OVERLAY")
    if Mancer.Util and Mancer.Util.ApplyFont then
        Mancer.Util.ApplyFont(label, 9)
    else
        label:SetFontObject(GameFontHighlightSmall)
    end
    if vertical then
        label:SetPoint("BOTTOM", frame, "TOP", 0, 1)
    else
        label:SetPoint("RIGHT", frame, "LEFT", -2, 0)
    end
    label:SetText(opts.label or "α")
    label:SetTextColor(1, 0.9, 0.55, 1)
    frame.label = label

    local function ClampA(a)
        a = tonumber(a)
        if not a then
            a = 1
        end
    ...
    local function ReadAlpha()
        if dbKey and MancerDB then
            return ClampA(MancerDB[dbKey])
        end
        return 1
    end

    local function WriteAlpha(a)
        a = ClampA(a)
        -- Keep fine precision while dragging (label still shows whole %).
    ...
    function frame:RefreshThumb()
        local a = ReadAlpha()
        local span = maxA - minA
        if span < 0.001 then
            span = 1
        end
        local ratio = (a - minA) / span
        if ratio < 0 then
            ratio = 0
        elseif ratio > 1 then
            ratio = 1
        ...
        fill:ClearAllPoints()
        if vertical then
            local y = ratio * length
            thumb:SetPoint("CENTER", track, "BOTTOM", 0, y)
            fill:SetPoint("BOTTOMLEFT", track, "BOTTOMLEFT", 0, 0)
            fill:SetPoint("BOTTOMRIGHT", track, "BOTTOMRIGHT", 0, 0)
            fill:SetHeight(math.max(1, y))
        else
            local x = ratio * length
        ...
        if label then
            label:SetText(string.format("%d%%", math.floor(a * 100 + 0.5)))
        end
    end
    ...
    -- Map cursor along the configured track length (not GetWidth/Height — those
    -- are often 0/tiny before layout, which made drags snap 15% ↔ 100%).
    local function AlphaFromCursor()
        ...
        if not left or not bottom or length <= 0 then
            return ReadAlpha()
        end
        local ratio
        if vertical then
            ratio = (cy - (bottom + pad)) / length
        else
            ratio = (cx - (left + pad)) / length
        end
        if ratio < 0 then
            ratio = 0
        elseif ratio > 1 then
        ...
        return minA + (maxA - minA) * ratio
    end

    local function StartDrag()
        frame.dragging = true
        WriteAlpha(AlphaFromCursor())
    end
