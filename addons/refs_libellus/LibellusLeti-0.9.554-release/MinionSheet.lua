...
local DISPLAY_CREATURE_ID = {
    ...
    skeletal_mage = 50075, -- Raise: Skeletal Mage (spell 500331 EffectMisc)
    tomb_king = 50320, -- Animate: Tomb King (GUID 0xF13000C490…)
    banshee = 500650, -- Raise: Banshee summon creature
    decaying_colossus = 50115, -- Raise: Decaying Colossus (spell 500989 EffectMisc)
}
local NAMEPLATE_HINT = "Friendly plates on for pet tokens (names-only paused while sheet open)"
...
local function PauseNamesOnlyForSheet()
    local hud = Mancer.MinionHpHudModule or Mancer.MinionHpHud
    if hud and hud.ClearNameplateVisualHide then
        hud:ClearNameplateVisualHide()
    end
end
...
function MinionSheet:BoostNameplates()
    ...
    -- Suspend capital mute while the sheet needs plate tokens.
    ...
    if self.nameplateCvarBackup then
        ...
        if Mancer.PersistNameplateCvarBackup then
            Mancer.PersistNameplateCvarBackup(self.nameplateCvarBackup)
        end
        -- Names-only strip breaks Ascension plate→unit binding; pause it while the sheet is open.
        PauseNamesOnlyForSheet()
        return
    end
    local hud = Mancer.MinionHpHud
    -- Prefer HUD's real user prefs if the HP list already forced plates on.
    if hud and hud.nameplateCvarBackup then
        local copy = {}
        for key, val in pairs(hud.nameplateCvarBackup) do
            copy[key] = val
        end
        if next(copy) then
            self.nameplateCvarBackup = copy
            if Mancer.PersistNameplateCvarBackup then
                Mancer.PersistNameplateCvarBackup(copy)
            end
            for key, enable in pairs(NAMEPLATE_ENABLE_VALUES) do
                pcall(SetCVar, key, enable)
            end
    ...
    if next(backup) then
        -- Always keep sheet backup so close can restore, even if already on.
        self.nameplateCvarBackup = backup
        if Mancer.PersistNameplateCvarBackup then
            Mancer.PersistNameplateCvarBackup(backup)
        end
        if not changed then
            -- Already on with no prior HUD prefs — still force-enable for tokens.
            for key, enable in pairs(NAMEPLATE_ENABLE_VALUES) do
                pcall(SetCVar, key, enable)
            end
        end
...
function MinionSheet:RestoreNameplates()
    ...
    InvalidateAdvisorScan()
    -- Scrub HUD backup if it only captured our forced-on state (nested boost bug).
    local hud = Mancer.MinionHpHud
    if hud and hud.nameplateCvarBackup then
        local onlyForced = true
        for key, enable in pairs(NAMEPLATE_ENABLE_VALUES) do
            local b = hud.nameplateCvarBackup[key]
            if b == nil or tostring(b) ~= tostring(enable) then
                onlyForced = false
                break
            end
        end
        if onlyForced then
            hud.nameplateCvarBackup = nil
        end
    end
    -- Force sync so throttle cannot leave a stale HUD boost pending; restores names-only if enabled.
    if hud and hud.SyncNameplateSupport then
        hud:SyncNameplateSupport(true)
    end
    -- Re-apply capital mute after sheet closes (backup kept while sheet was open).
    if Mancer.UpdateCapitalNameplateMute then
        Mancer.UpdateCapitalNameplateMute()
    end
end
...
function MinionSheet:ResolveTypeEntry(force)
    ...
    local function attachUnit(entry)
        ...
        if entry.unit and Advisor and Advisor.CanBindSheetUnit and not Advisor:CanBindSheetUnit(entry.unit) then
            entry.unit = nil
            entry.guid = nil
        elseif entry.unit and Advisor and not Advisor.CanBindSheetUnit and Advisor.IsOwnedByPlayer and not Advisor:IsOwnedByPlayer(entry.unit) then
            entry.unit = nil
            entry.guid = nil
        end
        return entry
...
function MinionSheet:UpdateModel(unit, minionId, forceRemount)
    ...
    local unitOk = unit and UnitExists and UnitExists(unit)
        and (not UnitIsDead or not UnitIsDead(unit))
    local unitGuid = unitOk and UnitGUID and UnitGUID(unit) or nil
    -- Prefer live unit mesh. Ascension SetCreature IDs (e.g. Mage 50075) often mount blank.
    local mountKey = (unitGuid and ("unit:" .. tostring(unitGuid)))
        or (creatureId and ("creature:" .. creatureId))
        or nil

    if not unitOk and not creatureId then
    ...
    local usedCreature = false
    local ok = pcall(function()
        model:SetScript("OnUpdate", nil)
        model:ClearModel()
        if unitOk then
            model:SetUnit(unit)
        elseif creatureId and model.SetCreature then
    ...
    -- Live unit failed — fall back to creature display id if we have one.
    if not ok and unitOk and creatureId and model.SetCreature then
        ok = pcall(function()
            model:SetScript("OnUpdate", nil)
            model:ClearModel()
            model:SetCreature(creatureId)
        ...
        if ok then
            mountKey = "creature:" .. creatureId
        end
    end

    if ok then
        self.modelGuid = mountKey
        self.modelCreatureId = usedCreature and creatureId or nil
        frame.modelHint:Hide()
    else
        self.modelGuid = nil
        self.modelCreatureId = nil
...
function MinionSheet:ShowWaitingPage(titleText, subtitleText)
    ...
    if frame.waitTitle then
        frame.waitTitle:SetText(titleText or "Waiting for a visible minion")
    end
    ...
    if frame.waitLine2 then
        frame.waitLine2:SetText(subtitleText or WAIT_HINT)
    end
    -- Re-assert Friends/Guardians/Pets while waiting so tokens can appear.
    if self.BoostNameplates then
...
function MinionSheet:Refresh(forceModel)
    ...
    local unitOk = unit and UnitExists and UnitExists(unit)
        and (not UnitIsDead or not UnitIsDead(unit))
    local unitGuid = (unitOk and UnitGUID and UnitGUID(unit)) or sheet.guid
    -- Match UpdateModel mount keys (live unit preferred over SetCreature).
    local mountKey = (unitGuid and ("unit:" .. tostring(unitGuid)))
        or (creatureId and ("creature:" .. creatureId))
        or nil
