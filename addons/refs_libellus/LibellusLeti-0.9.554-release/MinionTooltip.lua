...
local LF_MINION_TIPS = {
    ...
    decaying_colossus = {
        verdict = "situational",
        headline = "Costs 3 Life Force — personal damage sponge",
        lines = {
            "While up, 25% of damage you take is shifted to the Colossus.",
            "Does not enable Army of the Dead — use Raise: Abomination for that.",
            "Take for survivability, not as your boss DPS Raise.",
        },
    },
    crypt_fiend = {
        verdict = "math_first",
...
local function GetAdvisor()
    -- Prefer live instance (aura/LF state); fall back to module table for methods.
...
local function GetCalibratedUnitDps(minionId, fallback)
    ...
    if MinionDps and MinionDps.GetDpsEstimates then
        local ok, estimates = pcall(function()
            return MinionDps:GetDpsEstimates()
        end)
        if ok then
...
local function AddVerdictHeader(tooltip, verdict, headline)
    ...
    if info then
        ...
        if headline and headline ~= "" and TipLineRedundant(headline, { verdictLabel }) then
            tooltip:AddLine(
                string.format("|cff33ccff%s:|r %s", Mancer.SHORT_NAME or "Libellus Leti", verdictLabel),
        ...
        tooltip:AddLine("|cff33ccff" .. (Mancer.SHORT_NAME or "Libellus Leti") .. "|r", TOOLTIP_COLOR[1], TOOLTIP_COLOR[2], TOOLTIP_COLOR[3])
    end
...
function MinionTooltip:AugmentTalentTooltip(tooltip, spellId, spellName, tooltipTitle, requireTitleMatch)
    if not ShouldAugmentTooltips() or not tooltip then
        return false
    end
    ...
    if not ok then
        if not MinionTooltip._loggedTalentTipError and Mancer and Mancer.Print then
            MinionTooltip._loggedTalentTipError = true
            Mancer.Print("|cffff6666Libellus talent tip error:|r " .. tostring(result))
        end
        return false
    end
...
function MinionTooltip:TryAugmentFromTooltipDisplay(tooltip)
    ...
    if LooksLikeTalentTooltip(tooltip) and IsNecromancerAbility(spellId, spellName, tooltipTitle) then
        ...
        if tooltip.mancerAugmentKey ~= augmentKey then
            tooltip.mancerAugmentKey = augmentKey
            tooltip:AddLine(" ")
            tooltip:AddLine("|cff33ccff" .. (Mancer.SHORT_NAME or "Libellus Leti") .. ":|r No paper DPS model yet", MUTED_COLOR[1], MUTED_COLOR[2], MUTED_COLOR[3])
...
function MinionTooltip:AugmentTooltip(tooltip, spellId, spellName, tooltipTitle)
    ...
    local ok, err = pcall(function()
        ...
        local minionId = nil
        if Advisor then
            minionId = ResolveMinionId(spellId, spellName)
            if not minionId and tooltipTitle then
                minionId = ResolveMinionId(nil, tooltipTitle)
            end
        end

        -- Decaying Colossus is not the Abom/AotD package — talent tip only (no fake Abom loadout).
        local tipKeyEarly = ResolveTalentName(spellId, spellName, tooltipTitle, false)
        if tipKeyEarly == "Raise: Decaying Colossus" or minionId == "decaying_colossus" then
            if self:AugmentTalentTooltip(tooltip, spellId, spellName, tooltipTitle, false) then
                return
            end
        end

        -- Non-minion Animation talents: tip pack only (no MinionDps required).
        ...
        if not MinionDps then
            -- Still show talent tip for Raise/Animate if DPS module is unavailable.
            if self:AugmentTalentTooltip(tooltip, spellId, spellName, tooltipTitle, false) then
                return
            end
            return
        ...
        local estOk, a, _, c = pcall(function()
            return MinionDps:GetDpsEstimates()
        end)
        if estOk then
            estimates = a
        ...
        local row = estimates and estimates[minionId]
        local lfCost = Advisor and Advisor.GetMinionLifeForceCost and Advisor:GetMinionLifeForceCost(minionId)
        local label = GetTooltipCreatureLabel(
            spellName,
            (MinionDps.GetMinionLabel and MinionDps:GetMinionLabel(minionId)) or minionId
        )
        local requiredLevel = Advisor and Advisor.GetMinionRequiredLevel and Advisor:GetMinionRequiredLevel(minionId)
        ...
        if requiredLevel then
            ...
            elseif Advisor.HasMinionTalent and not Advisor:HasMinionTalent(minionId) then
        ...
        if row and row.dps and row.dps > 0 then
            if lfCost and lfCost > 0 then
                tooltip:AddDoubleLine(
                    "|cff33ccff" .. (Mancer.SHORT_NAME or "Libellus Leti") .. "|r DPS/LF",
                ...
                tooltip:AddDoubleLine(
                    "|cff33ccff" .. (Mancer.SHORT_NAME or "Libellus Leti") .. "|r DPS",
                    string.format("%.0f", row.dps),
                    TOOLTIP_COLOR[1], TOOLTIP_COLOR[2], TOOLTIP_COLOR[3],
                    VALUE_COLOR[1], VALUE_COLOR[2], VALUE_COLOR[3]
                )
            ...
            if source == "benchmark" then
                local ctx = "Built-in ST placeholder — not AoE/pack DPS"
                if MinionDps.GetBenchmarkContextLabel then
                    local ctxOk, ctxVal = pcall(function()
                        return MinionDps:GetBenchmarkContextLabel()
                    end)
                    if ctxOk and type(ctxVal) == "string" and ctxVal ~= "" then
                        ctx = ctxVal
                    end
                end
                tooltip:AddLine(ctx, 0.5, 0.5, 0.5)
            elseif source then
                tooltip:AddLine("Source: " .. tostring(source), 0.5, 0.5, 0.5)
            end
        elseif Advisor and Advisor.UsesTemporaryTracking and Advisor:UsesTemporaryTracking(minionId) then
            tooltip:AddLine("|cff33ccff" .. (Mancer.SHORT_NAME or "Libellus Leti") .. ":|r CD minion (not LF-limited)", 0.8, 0.8, 0.8)
            tooltip:AddLine("Summon on cooldown alongside your LF loadout", 0.55, 0.55, 0.55)
        else
            tooltip:AddLine("|cff33ccff" .. (Mancer.SHORT_NAME or "Libellus Leti") .. ":|r No DPS data yet", 0.8, 0.8, 0.8)
    ...
    if not ok and err and Mancer and Mancer.Print then
        -- Surface once so broken DPS estimates cannot silently kill GameTooltip.
        if not MinionTooltip._loggedAugmentError then
            MinionTooltip._loggedAugmentError = true
            Mancer.Print("|cffff6666Libellus tooltip error:|r " .. tostring(err))
        end
    end
end
...
function MinionTooltip:Init()
    ...
    MancerDB.minionDps = MancerDB.minionDps or {}
    -- Default comes from Core DEFAULTS / welcome wizard; do not force ON.

    -- Re-ensure hooks even after a prior Init (GameTooltip may not have existed yet).
