...
local EMBEDDED_VERSION = "0.9.554"
...
-- Feature toggles default OFF for new installs; the welcome wizard turns picks on.
-- CopyDefaults only fills nil — existing saved profiles keep their values.
local DEFAULTS = {
    showMana = false,
    showHealth = false,
    showManaBar = false,
    ...
    showZombieCounter = false,
    showMinionHpList = false,
    -- "Life Force (n/max)" HUD label — independent of stance prompts.
    showLifeForceText = false,
    hideMinionHpNameplateVisuals = false,
    -- Busy capitals (Orgrimmar, Stormwind, …): mute plate CVars for FPS.
    disableNameplatesInCapitals = false,
    -- Hide plate bars / keep name text (Ascension NamePlateDriver).
    ...
    anchorX = 0,
    anchorY = 80,
    -- Arc size uses the classic radius; "start bigger" is default scale + icon bases.
    arcRadius = 65,
    scale = 1.35,
    ...
    -- Comfortable on-screen size at first enable (was 0.75 / 0.85 / 0.8 — too tiny under LF).
    animateIconScale = 1.25,
    zombieIconScale = 1.15,
    procIconScale = 1.15,
    ...
    -- Keep minimap on so Hub is always findable without knowing /leti.
    minimap = {
        hide = false,
        angle = 220,
    },
...
local function MigrateSavedVariables()
    ...
    -- Minimap stays reachable by default (Hub without needing /leti).
    ...
    -- Undo 0.9.554 blank-default hide for anyone who has not finished the wizard yet.
    if not MancerDB.minimapReachable_v554 then
        MancerDB.minimapReachable_v554 = true
        if not MancerDB.welcomeWizardDone_v1 then
            MancerDB.minimap.hide = false
        end
    end
    ...
    -- One-shot: bump Animate / zombie / proc icons off the old tiny defaults.
    if not MancerDB.hudIconComfortSize_v555 then
        MancerDB.hudIconComfortSize_v555 = true
        local function near(v, target)
            v = tonumber(v)
            return v ~= nil and math.abs(v - target) < 0.02
        end
        if MancerDB.animateIconScale == nil or near(MancerDB.animateIconScale, 0.75) then
            MancerDB.animateIconScale = 1.25
        end
        if MancerDB.zombieIconScale == nil or near(MancerDB.zombieIconScale, 0.85) then
            MancerDB.zombieIconScale = 1.15
        end
        if MancerDB.procIconScale == nil or near(MancerDB.procIconScale, 0.8) then
            MancerDB.procIconScale = 1.15
        end
    end

    -- One-shot: larger default mana/health/runic vines (65 → 90 → 135).
    -- Kept as a no-op marker; v560 reverts the radius blow-up below.
    if not MancerDB.hudBarComfortSize_v556 then
        MancerDB.hudBarComfortSize_v556 = true
    end
    MancerDB.hudBarComfortSize_v555 = true

    -- Undo 90/135 comfort radius migrations — those multiplied on top of saved
    -- HUD scale and blew up old layouts. Keep bigger starts via scale/icon defaults.
    if not MancerDB.hudBarRadiusRestore_v560 then
        MancerDB.hudBarRadiusRestore_v560 = true
        local r = tonumber(MancerDB.arcRadius)
        if r ~= nil and (math.abs(r - 90) < 0.5 or math.abs(r - 135) < 0.5) then
            MancerDB.arcRadius = 65
        end
    end

    -- Life Force HUD text is now its own toggle. Older builds always showed it
    -- with the Animation advisor — keep that for existing profiles.
    if not MancerDB.lifeForceTextToggle_v561 then
        MancerDB.lifeForceTextToggle_v561 = true
        if MancerDB.showLifeForceText == nil then
            if MancerDB.welcomeWizardDone_v1
                or MancerDB.seenPlayerHelp_v1
                or MancerDB.scale ~= nil
                or MancerDB.arcRadius ~= nil then
                MancerDB.showLifeForceText = true
            end
        end
    end
...
-- True when the player has turned on real features (not just default SV fill).
-- Minimap-on and migration flags alone do NOT count — fresh installs get those via DEFAULTS.
function Mancer.HasCustomizedSettings()
    local db = MancerDB
    ...
    for i = 1, #featureKeys do
        if db[featureKeys[i]] == true then
            return true
        end
    end
    local plates = db.nameplateNamesOnlyTargets
    if type(plates) == "table" then
        if plates.guardians or plates.players or plates.npcs or plates.enemies then
            return true
        end
    end
    if type(db.necromancer) == "table" and db.necromancer.stanceEnabled == true then
        return true
    end
    if type(db.minionDps) == "table" and db.minionDps.tooltipEnabled == true then
        return true
    end
    return false
end

function Mancer.IsWelcomeWizardDone()
    return MancerDB and MancerDB.welcomeWizardDone_v1 == true
end

function Mancer.MarkWelcomeWizardDone()
    MancerDB = MancerDB or {}
    MancerDB.welcomeWizardDone_v1 = true
    -- Keep legacy help flag in sync so old tips stay quiet.
...
local function HandleSlashCommand(msg)
    ...
    if lower == "welcome reset" or lower == "setup reset" or lower == "help reset" then
        MancerDB = MancerDB or {}
        MancerDB.welcomeWizardDone_v1 = nil
        MancerDB.seenPlayerHelp_v1 = nil
        if Mancer.WelcomeWizard then
            Mancer.WelcomeWizard._shownThisSession = nil
            if Mancer.WelcomeWizard.Open then
                Mancer.WelcomeWizard:Open(true)
            end
        end
        if Mancer.Print then
            Mancer.Print("Welcome wizard reset. Pick what you want from Leti.")
        end
        return
    end

    if lower == "help" or lower == "guide" or lower == "tutorial" or lower == "start"
        or lower == "gettingstarted" or lower == "welcome" or lower == "setup" then
        if Mancer.WelcomeWizard and Mancer.WelcomeWizard.Open then
            Mancer.WelcomeWizard:Open(true)
        elseif Mancer.Hub and Mancer.Hub.OpenHelp then
            Mancer.Hub:OpenHelp()
        else
            Mancer.OpenHub()
        end
...
eventFrame:SetScript("OnEvent", function(_, event, arg1)
    ...
    if event == "PLAYER_ENTERING_WORLD" or event == "ZONE_CHANGED_NEW_AREA" then
        ...
        -- Show welcome after world load (more reliable than PLAYER_LOGIN alone).
        if event == "PLAYER_ENTERING_WORLD"
            and Mancer.WelcomeWizard
            and Mancer.WelcomeWizard.TryShow
            and not Mancer._welcomeWizardTriedEnteringWorld then
            Mancer._welcomeWizardTriedEnteringWorld = true
...
local OPTION_TIPS = {
    ...
    showLifeForceText = {
        title = "Show Life Force count",
        lines = {
            "Shows the Life Force (free/max) label near your character (same spot as stance text).",
            "Independent of Undead stance prompts — turn either on without the other.",
            "Animation Necromancer only. Drag the T handle in move mode to reposition.",
        },
    },
    showMinionHpList = {
...
function Options:SyncControls()
    ...
    if self.rowLifeForceText then
        self.rowLifeForceText.checkbox:SetChecked(MancerDB.showLifeForceText == true and 1 or nil)
    end
...
function Options:CreatePanel()
    ...
    do
        ...
        self.rowLifeForceText = CreateCheckboxRow(dispRight, L["OPT_SHOW_LIFE_FORCE"] or "Show Life Force count", self.rowZombieCounter, -2, "showLifeForceText", 248)
        self.rowMinionHpList = CreateCheckboxRow(dispRight, L["OPT_SHOW_MINION_HP"] or "Show minion HP bars", self.rowLifeForceText, -2, "showMinionHpList", 248)
