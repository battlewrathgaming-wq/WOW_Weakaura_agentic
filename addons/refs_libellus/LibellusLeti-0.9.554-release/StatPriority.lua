...
-- Ordered gearing steps. Caps with targetPct are checked live; intellect/SP is the
-- primary stack after spell hit (no numeric soft cap). Haste is a later soft aim —
-- Animation damage coeffs and Necromancy army power scale hard from INT+SP.
-- Sources:
--   spell hit 17%  — Ascension Stats wiki (boss +3 hard cap)
--   intellect/SP   — primary throughput (ability coeffs, Necromancy AP/SP, crit from INT)
--   haste 12%      — early comfort soft (after INT/SP, not before)
...
StatPriority.BREAKPOINTS = {
    {
        ...
        source = "Client gtCombatRatings.dbc (DBC-backed)",
    },
    {
        id = "intellect",
        label = "Intellect / Spell Power",
        kind = "primary",
        why = "Best Animation scales so far — feeds ability coeffs, Necromancy army power, and spell crit.",
        source = "Live play + Animation INT/SP tooltip coefficients + PaperMath",
    },
    {
        id = "hasteEarly",
        label = "Haste (early)",
        ...
        ratingTableKey = "spellHaste",
        ratingName = "haste rating",
        targetPct = 12,
        why = "Comfort soft after INT/SP — casts/GCD smoother; weaker than INT/SP for army damage.",
        source = "Community early-comfort pattern (deprioritized vs INT/SP for Animation)",
    },
    {
        id = "crit",
        label = "Crit",
...
--- First unmet hard/soft step. Intellect/SP becomes active once spell hit is met;
--- haste/crit soft caps ride along as unfinished checklist items.
    ...
    for _, step in ipairs(self.BREAKPOINTS) do
        ...
        elseif not self:IsStepMet(step, stats) then
            if not primaryStep then
                -- Still before INT/SP — spell hit gate
                earlyGatesMet = false
                if not active then
                    active = step
                end
...
function StatPriority:PrintGuide()
    ...
    for i, step in ipairs(self.BREAKPOINTS) do
        ...
        if step.kind == "primary" then
            if activeId == step.id then
                ...
                status = " (primary when spell hit is capped)"
            end
        else
            local live = stats[step.stat] or 0
    ...
    if active then
        if active.kind == "primary" then
            Mancer.Print("Next aim: stack Intellect and Spell Power (best Animation scales so far).")
...
function StatPriority:ShowPanel()
    ...
    if Mancer.Print then
        Mancer.Print("Open Libellus Leti Hub → Guides → Stat Priority.")
    end
end
