...
function CombatLog.Parse(...)
    ...
    local spellId, spellName, amount, missType = nil, nil, 0, nil
    ...
    local function PickMissType(...)
        for i = 1, select("#", ...) do
            local v = select(i, ...)
            if type(v) == "string" and KNOWN_MISS[string.upper(v)] then
    ...
    elseif token == "SWING_MISSED" then
        -- Payload varies by client: missType is usually A1; scan A1–A4 to be safe.
        missType = PickMissType(A1, A2, A3, A4)
    elseif token == "SPELL_DAMAGE" or token == "SPELL_PERIODIC_DAMAGE" or token == "RANGE_DAMAGE" then
        spellId = tonumber(A1)
        spellName = A2
        amount = ExtractDamageAmount(token, A1, A2, A3, A4, A5)
    elseif token == "SPELL_MISSED" or token == "RANGE_MISSED" or token == "SPELL_PERIODIC_MISSED" then
        -- Payload: spellId, spellName, spellSchool, missType [, amount]
        spellId = tonumber(A1)
        spellName = A2
        missType = PickMissType(A1, A2, A3, A4, A5, A6)
