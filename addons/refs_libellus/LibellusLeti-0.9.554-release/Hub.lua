...
local DPS_FALLBACK_ICONS = {
    ...
    skeletal_mage = "Interface\\Icons\\Spell_Frost_FrostBolt02",
    ...
    decaying_colossus = "Interface\\Icons\\Achievement_Dungeon_GrimBatol_Drahga_the_Shadowburner",
    other_owned = "Interface\\Icons\\Spell_Shadow_RaiseDead",
    ...
    ["Blight"] = "Interface\\Icons\\Spell_Shadow_DeathAndDecay",
    ["Expunge Blight"] = "Interface\\Icons\\Spell_Shadow_AbominationExplosion",
    ["Expunge from Blight"] = "Interface\\Icons\\Spell_Shadow_AbominationExplosion",
    expunge_blight = "Interface\\Icons\\Spell_Shadow_AbominationExplosion",
...
function Hub:PopulateDpsAccordions(page, data, minionW, playerW)
    ...
    for _, row in ipairs(minionRows) do
        ...
        local hitMiss = (Mancer.MinionDpsModule and Mancer.MinionDpsModule.FormatHitsMisses)
            and Mancer.MinionDpsModule:FormatHitsMisses(row.hits, row.misses, row.missTypes)
            or string.format("%d hits", row.hits or 0)
        acc:SetHeader(
            row.iconSpellId,
            string.format(
                "%s · %.0f DPS/unit · %s%s",
        ...
        if row.actors and #row.actors > 0 then
            for _, actor in ipairs(row.actors) do
                local actorHit = (Mancer.MinionDpsModule and Mancer.MinionDpsModule.FormatHitsMisses)
                    and Mancer.MinionDpsModule:FormatHitsMisses(actor.hits, actor.misses, actor.missTypes)
                    or string.format("%d hits", actor.hits or 0)
                acc:AddBodyLine(0, nil, string.format(
                    "%s · %s dmg · %s · %.0f DPS",
        ...
        for _, spell in ipairs(row.spells or {}) do
            local spellHit = (Mancer.MinionDpsModule and Mancer.MinionDpsModule.FormatHitsMisses)
                and Mancer.MinionDpsModule:FormatHitsMisses(spell.hits, spell.misses, spell.missTypes)
                or string.format("%d hits", spell.hits or 0)
            acc:AddBodyLine(
                4,
                spell.spellId,
                string.format(
    ...
    if #players == 0 then
        ...
        acc:SetHeader(nil, "(no player damage this fight)")
        acc:UpdateHeight()
    else
        for _, row in ipairs(players) do
            ...
            elseif row.kind == "damage"
                or row.kind == "spell"
                or ((row.damage or 0) > 0 and row.kind ~= "dot")
                or ((row.casts or 0) > 0 and row.id == "march_of_the_dead") then
                ...
                if row.sessionNote then
                    ...
                    if (row.targetCount or 0) > 0 then
                        acc:AddBodyLine(0, nil, string.format(
                            "%d target%s",
                            row.targetCount or 0,
                            (row.targetCount or 0) == 1 and "" or "s"
                        ))
                    end
            ...
            elseif row.sessionNote then
                ...
                -- DoT uptime rows; include damage/DPS when ticks were recorded.
...
function Hub:RefreshLocalizedUI()
    TAB_DEFS = GetTabDefs()
    for _, def in ipairs(TAB_DEFS) do
        local btn = self.tabButtons and self.tabButtons[def.id]
        if btn then
            if btn.SetText then
            ...
            if btn.subtitle and def.subtitle then
                btn.subtitle:SetText(def.subtitle)
            end
            BindButtonTooltip(btn, def.label, def.tip)
        end
    end
    local credits = self.pages and self.pages.credits
    if credits then
        if credits.creditsHeader and credits.creditsHeader.label and credits.creditsHeader.label.SetText then
            credits.creditsHeader.label:SetText(Mancer.Loc("HUB_CREDITS_TITLE", "Thanks"))
        end
        if credits.creditsBody and credits.creditsBody.SetText then
            credits.creditsBody:SetText(Mancer.Loc(
                "HUB_CREDITS_BODY",
                credits.creditsBody:GetText() or ""
            ))
        end
    end
end

function Hub:HasDismissedPlayerHelp()
    return (Mancer.IsWelcomeWizardDone and Mancer.IsWelcomeWizardDone())
        or (MancerDB and MancerDB.seenPlayerHelp_v1 == true)
end

function Hub:DismissPlayerHelp()
    if Mancer.MarkWelcomeWizardDone then
        Mancer.MarkWelcomeWizardDone()
    else
        MancerDB = MancerDB or {}
        MancerDB.seenPlayerHelp_v1 = true
    end
end

function Hub:SyncHelpSkipButton()
    -- Help UI moved to WelcomeWizard; kept as a no-op for callers.
end

function Hub:OpenHelp()
    if Mancer.WelcomeWizard and Mancer.WelcomeWizard.Open then
        Mancer.WelcomeWizard:Open(true)
        return
    end
    self:Open()
    if self.frame and self.frame:IsShown() then
        self:SelectTab("credits")
    end
end
        ...
        if stats and Mancer.StatPriorityModule.GetActiveStep then
            ...
            if active then
                if active.kind == "primary" then
                    table.insert(parts, "Next: INT/SP")
...
function Hub:SetPageButtonActive(pageKey, mapKey, activeKey, id)
    local page = self.pages and self.pages[pageKey]
    local map = page and page[mapKey]
    if not map then
        return
    end
    page[activeKey] = id
...
function Hub:SetCombatButtonActive(id)
    self:SetPageButtonActive("combat", "combatButtons", "activeCombatButton", id)
end

function Hub:PrepareTabDetail(tabId, extraHideKeys)
    if self.viewMode == "sheet" then
        self:DetachMinionSheet()
    end
    self.viewMode = nil
    if self.pages then
        ...
        for _, key in ipairs(extraHideKeys or {}) do
    ...
    local page = self.pages and self.pages[tabId]
    if page then
...
function Hub:PrepareCombatDetail()
    return self:PrepareTabDetail("combat")
end

function Hub:FillDetailBody(page, title, lines)
...
function Hub:ShowCombatDetailReport(title, lines, buttonId)
    ...
    if not page or not page.detailTitle or not page.detailBody then
        if Mancer.Print then
            Mancer.Print(title or (Mancer.DISPLAY_NAME or "Libellus Leti"))
            for _, line in ipairs(lines or { "(empty)" }) do
                Mancer.Print(tostring(line))
            end
        end
        return
    end
    ...
    self:FillDetailBody(page, title, lines)
end
...
function Hub:ShowStVsAoe()
    ...
    if not page or not page.stIntro then
        self:ShowCombatDetailReport("ST vs AOE", { "Combat page not ready." }, "stAoe")
        return
    end
...
-- Compatibility shim: all Hub reports go through the Combat detail pane.
function Hub:ShowInHubReport(title, lines)
    self:ShowCombatDetailReport(title, lines)
end
...
function Hub:SetTheoryButtonActive(id)
    self:SetPageButtonActive("theory", "theoryButtons", "activeTheoryButton", id)
end

function Hub:PrepareTheoryDetail()
    return self:PrepareTabDetail("theory", { "statPriority" })
end
...
function Hub:ShowTheoryDetailReport(title, buttonId, fn)
    ...
    if not page or not page.detailTitle or not page.detailBody then
        self:ShowCombatDetailReport(title, self:CaptureReportLines(fn))
        return
    end
    if buttonId then
    ...
    self:FillDetailBody(page, title, self:CaptureReportLines(fn))
end
...
function Hub:ShowDpsExport(text, statusMsg)
    ...
    if not page or not page.detailTitle then
        self:ShowCombatDetailReport("Save DPS", { statusMsg or "Export ready.", text or "" }, "save")
        return
    end
...
function Hub:ResetDpsSession()
    if not Mancer.MinionDpsModule then
        self:Notify("Minion DPS module not loaded.")
        return
    end
    Mancer.MinionDpsModule:ResetSession()
    self:Notify("Minion DPS cleared — saved fights and current pull wiped.")
    -- Refresh the open combat panel so stale numbers (e.g. 87k DPS/unit) disappear.
    local active = self.activeCombatButton
    if active == "dps" or active == "session" or active == "save" or active == "resetDps" then
        self:ShowMinionDps("auto")
    elseif active == "combo" then
        self:ShowCombo()
    elseif active == "stAoe" then
        self:ShowStVsAoe()
    end
end
...
GetTabDefs = function()
    return {
        {
            id = "combat",
            label = Mancer.Loc("HUB_TAB_COMBAT", "Combat"),
            subtitle = Mancer.Loc("HUB_TAB_COMBAT_SUB", "Army · LF · DPS"),
            tip = Mancer.Loc("HUB_TAB_COMBAT_TIP", "Army picks, Life Force, DPS, and ST vs AoE"),
            icon = "Interface\\Icons\\Spell_Shadow_DeathCoil",
        },
        {
            id = "minions",
            label = Mancer.Loc("HUB_TAB_MINIONS", "Minions"),
            subtitle = Mancer.Loc("HUB_TAB_MINIONS_SUB", "Guardian sheet"),
            tip = Mancer.Loc("HUB_TAB_MINIONS_TIP", "Live guardian sheet — stats and model"),
            icon = "Interface\\Icons\\Spell_Shadow_AnimateDead",
        },
        {
            id = "theory",
            label = Mancer.Loc("HUB_TAB_GUIDES", "Guides"),
            subtitle = Mancer.Loc("HUB_TAB_GUIDES_SUB", "Stats · Paper · Buffs"),
            tip = Mancer.Loc("HUB_TAB_GUIDES_TIP", "Stat caps, paper DPS, and buff picks"),
            icon = "Interface\\Icons\\INV_Misc_Book_09",
        },
        {
            id = "setup",
            label = Mancer.Loc("HUB_TAB_SETUP", "Setup"),
            subtitle = Mancer.Loc("HUB_TAB_SETUP_SUB", "Bars · Display"),
            tip = Mancer.Loc("HUB_TAB_SETUP_TIP", "Open display options (bars, ticks, fonts, layout)"),
            icon = "Interface\\Icons\\INV_Misc_Gear_01",
        },
        {
            id = "credits",
            label = Mancer.Loc("HUB_TAB_CREDITS", "Credits"),
            subtitle = Mancer.Loc("HUB_TAB_CREDITS_SUB", "Thanks"),
            tip = Mancer.Loc("HUB_TAB_CREDITS_TIP", "Thanks to everyone who helped with Libellus Leti"),
            icon = "Interface\\Icons\\INV_Misc_GroupLooking",
        },
    }
end

-- Prefer post-ADDON_LOADED Apply; Create/Open also refresh so labels never stay as keys.
TAB_DEFS = GetTabDefs()
...
function Hub:Create()
    ...
    -- Locale.Apply runs on ADDON_LOADED; rebuild labels so we never bake HUB_TAB_* keys.
    TAB_DEFS = GetTabDefs()
    ...
    do
        ...
        local row1 = CreateButtonRow(page, header, -8, {
            ...
            { id = "save", text = "Save DPS", tooltip = "Copy last fight DPS to clipboard / text you can paste into a .txt file", onClick = function() Hub:SaveDpsFight() end },
            {
                ...
                text = "Reset",
                width = 72,
                tooltip = "Clear saved fights and the current pull so bad DPS numbers (e.g. inflated unit-time) do not stick around.",
                onClick = function() Hub:ResetDpsSession() end,
            },
            {
                id = "macros",
                text = "Install Macros",
    ...
    do
        ...
        local row1 = CreateButtonRow(page, header, -8, {
            ...
            },
            {
                id = "buffs",
                text = "Buffs",
                tooltip = "Preferred buffs: Grim Mandate, Razorice, Bone Ward by default; Glacial Ward for ghoul-heavy offense; Chill of the Tomb",
    ...
    -- ── Credits / Thanks (rail slot — geometry locked) ──
    do
        local page = CreatePage(frame)
        local header = CreateSection(page, Mancer.Loc("HUB_CREDITS_TITLE", "Thanks"), nil, 0)
        header:ClearAllPoints()
        header:SetPoint("TOPLEFT", page, "TOPLEFT", 0, 0)
        header:SetWidth(640)
        page.creditsHeader = header
        ...
        body:SetSpacing(3)
        body:SetText(Mancer.Loc(
            "HUB_CREDITS_BODY",
            (Mancer.DISPLAY_NAME or "Libellus Leti")
                .. " is built for casual Animation Necromancers on Ascension.\n\n"
...
function Hub:Open()
    ...
    if self.frame then
        self:RefreshLocalizedUI()
        self.frame:Show()
        if Mancer.WelcomeWizard and Mancer.WelcomeWizard.TryShow then
            Mancer.WelcomeWizard:TryShow()
        end
    end
end
