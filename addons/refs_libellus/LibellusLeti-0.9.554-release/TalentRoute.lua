...
-- These S/A names are confirmed in-game / WeakAuras / Libellus Leti tips but absent or
...
TalentRoute.OVERLAY_SPEC_ORDER = {
    ...
    "Summoning Prodigy",
    "Scourge Disciple",
    { name = "Animate: Bone Wraith", entryId = 10495 }, -- CA 10495 / spell 805032
    ...
    -- Rest: entryIds required — name-only picks were still marked NEXT after 1/1
    -- when the open-tree rank map missed the name key.
    { name = "Depravity", entryId = 5766 },
    { name = "Army of the Dead", entryId = 7298 },
    { name = "Animate: Tomb King", entryId = 30495 },
    { name = "Crypt Keeper", entryId = 7112 },
}
...
-- 9 AE gate cluster: Overwhelming Force, Reclaimed Life, Disciple, Putrid
-- Summoner. Early path: … Living Dead → Glacial Tap → Icecrown Scriptures 2/2
-- (9 AE) → OF. Skip Lich Essence (Lichfrost is free from Bone King).
-- Skip Fetid Ward / Command: Hook / Bone Tithe as route picks.
-- OF entryId 6897 / spell 531128 (29196 is Tears of Lordaeron).
TalentRoute.OVERLAY_CLASS_ORDER = {
    { name = "Hulking Hordes", entryId = 6885, ranks = 1 },           -- 1
    { name = "Vampiric Aura", entryId = 6458 },                       -- 2
    { name = "Life For Power", entryId = 6903 },                      -- 3  (OF connector)
    ...
    { name = "Glacial Tap", entryId = 29787 },                        -- 7
    { name = "Icecrown Scriptures", entryId = 6459, ranks = 1 },     -- 8
    { name = "Icecrown Scriptures", entryId = 6459, ranks = 2 },     -- 9  (9 AE)
    { name = "Overwhelming Force", entryId = 6897 },                  -- 10
    { name = "Reclaimed Life", entryId = 31048 },                     -- 11 (via OF)
    { name = "Disciple of Kel'thuzad", entryId = 33784, ranks = 1 },  -- 12 (via OF)
    { name = "Disciple of Kel'thuzad", entryId = 33784, ranks = 2 },  -- 13
    { name = "Mass Grave", entryId = 33778 },                        -- 14
    { name = "Lich Commander", entryId = 5781 },                     -- 15 (Putrid connector)
    { name = "Putrid Summoner", entryId = 6886 },                    -- 16 (9 AE + Lich Commander)
    { name = "Create Frozen Reliquary", entryId = 34980 },           -- 17
    { name = "Ner'zhul's Blessing", entryId = 31143 },               -- 18
    { name = "Death's Gate", entryId = 29788 },                      -- 19
    { name = "Improved Mandates", entryId = 34977 },                 -- 20
    { name = "Cobalt-Plated Armour", entryId = 6555 },               -- 21
    { name = "Winds of Northrend", entryId = 33733 },               -- 22
    { name = "Frost Runes", entryId = 33771 },                      -- 23
    { name = "Animate: Frost Wyrm", entryId = 30600 },              -- 24
}
...
function TalentRoute:GetRankForPick(name, entryId, rankMap)
    ...
    if idKey then
        ...
        if type(byId) == "number" and byId > 0 then
            return byId
        end
        if byId == true then
            return 1
        end
        -- Do not treat explicit 0 as final before checking the name key —
        -- CollectRankMap can store #id=0 while the name key still has the live rank.
...
-- Seed from GetKnownTalents, then apply open-tree extras on top.
-- Tree ranks win per key (including clearing on 0 after unlearn). Known list
-- fills gaps when the tree scan misses a name/#id — that was leaving Depravity
-- etc. stuck as NEXT after 1/1 when the route entry had no entryId.
function TalentRoute:BuildTakenRankMap(rankExtras)
    ...
    if Mancer.Ascension and Mancer.Ascension.GetKnownTalents then
        ...
        for _, talent in ipairs(Mancer.Ascension.GetKnownTalents() or {}) do
            -- Never default nil rank to 1 — GetKnownTalentEntries often omits rank,
            -- and `rank or 1` marked every known entry taken (skipped Master Animator → NEXT jumped ahead).
            local rank = tonumber(talent.rank) or 0
            if rank > 0 and (talent.name or talent.id) then
                note(talent.name, rank, talent.id)
            end
        end
    end
...
-- All remaining picks in queue order (for full-path overlay mode).
function TalentRoute:GetRemainingInOrder(order, rankMap)
    local out = {}
    if not order then
        return out
    end
    for _, entry in ipairs(order) do
        local name, need, entryId = ParseOrderEntry(entry)
        if name and not self:HasAtLeastRank(name, rankMap, need, entryId) then
            table.insert(out, { name = name, entryId = entryId, need = need })
        end
    end
    return out
end

-- Remaining free / class / spec queues + the same next picks as GetNextOverlayPicks.
function TalentRoute:GetOverlayPaths(rankExtras)
    local picks = self:GetNextOverlayPicks(rankExtras)
    local ranks = picks.taken or self:BuildTakenRankMap(rankExtras)
    ...
    return {
        free = freePath,
        class = self:GetRemainingInOrder(self.OVERLAY_CLASS_ORDER, ranks),
        spec = self:GetRemainingInOrder(self.OVERLAY_SPEC_ORDER, ranks),
        next = picks,
    }
end
