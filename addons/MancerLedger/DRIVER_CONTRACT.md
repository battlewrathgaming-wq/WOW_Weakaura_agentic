...
| `bucket.activeSeconds` | window close → += lived (:1739). Windows close on: UNIT_DIED (:1282), estimated expiry, or combat end. (SETTLED 2026-07-31 via /combatlog disk witness: UNIT_DIED FIRES for combat-killed pets on this fork — their died-close path is LIVE; only overwrite-despawn is silent.) | **observed/estimated unit-time, NOT lifetime** | denominator ONLY under the cadence gate (below) |
