...
| `bucket.activeSeconds` | window close → += lived (:1739). Windows close on: UNIT_DIED (:1282 — note: our petlog proved UNIT_DIED is SILENT for pets on this fork, so this path rarely fires), estimated expiry, or combat end | **observed/estimated unit-time, NOT lifetime** | denominator ONLY under the cadence gate (below) |
