...
4. `driver_bench_proposition.md`     the bench's own plan for items 1–3 — REVIEWED, holds
5. `driver_authoring_acceptance.md`  item 1 + item 2's first proof + adaptor: A1–A7 (build to
                                     these; each row names its mutation)
6. `driver_walk_acceptance.md`       the consumer's rule: W1–W7 (W1–W6 done; W7 awaits a Lua
                                     consumer)
...
## RULED SINCE THE PROPOSITION (not yet in older text)
- no refusal anywhere: `listen(UNIT_DIED, name)` — no name, nothing arms; editor TELLS
- boss beacon = tab reach + tab push-token, ANY above, kill = next (model §2c)
- the push token arrives at the DRIVER; background processes serve awake instructions, two-way
- R1 / R2 / R3 are Battlewrath's; Analyst positions on file (referenced-under/owned-in-pane ·
  default ±2.5 · test driver = mode of /dr walk). G2 → ordinal → G10 do NOT wait on them.

## HISTORY — read for WHY, never for WHAT to build
- `driver_design_advisory.md`   the arc's design as challenged; many sections superseded by
                                scoping / model (arm-zone, K-forward, `requires`, two artifacts,
                                re-seat-only, `mode: once|while`, `4.1 = child`) — banner inside
- `driver_analysis_asklist.md`  the reasoning space §A–§K + ledger; findings stand, designs in
                                §H are superseded where the model says otherwise
- `driver_analysis_brief.md`    the opening brief; §2 "never compute your own" OVERRULED (R-a)
