# DRAINED RECONCILE ITEMS — the settled conversation, moved out of the inbox 2026-08-22

_Moved at Battlewrath's instruction: **"You are free to preserve in history today's state and
refactor comms that best suits the comms needs."**_

⚠⚠ **WHY, MEASURED.** `Reconcile_inbox.md` had grown to 5,985 lines of which **4,376 — 73% — were
items already DRAINED.** The inbox's own convention says *"an item is EITHER a full entry in the
inbox OR a row in the log, never both"*, and it was both for **25 items**. ★ The guard the Analyst
wrote enforced only half of that rule — drained ⟶ has a log row — and never the removal.

⟶ **This is the second-copy fault a fifth time**, and by volume it is the largest: L18/L20, the
concept homes, the enumerated action list, `Routes.ACTIONS`, and this.

    THE CONCLUSION   `ANALYST_LOG.md` — one row each, IS · IS NOT · why · where. **The index.**
    THE REASONING    below, verbatim and unedited. **A record, not a basis.**
    THE HISTORY      git. Nothing here was deleted; §510 and earlier carry every version.

⚠ **THIS FILE GOVERNS NOTHING AND IS CITED BY NOTHING THAT BUILDS.** It is `history/` for the same
reason RI-31 moved five audits here on his word — *"more examples of the same text to infer
separately. Source of truth and pointers."* A finished item restating a settled conclusion is
another text an agent reads as CURRENT.

★ **Every item below was verified to hold a row in `ANALYST_LOG.md` BEFORE it was moved.** The
script refuses to move an item whose conclusion is not already indexed — the removal cannot lose
a decision.
