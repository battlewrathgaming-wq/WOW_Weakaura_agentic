...
# W6 — **DONE.** `/coadump st chain`, 193 seconds, six authored pins.

`records/20260817_170747_470__chain.json` · route generated by
`py addons/tools/emit_chain_route.py rfc_combat` from the landed capture (sha `f9092b22ff1a`)

    6 set · 6 clear · 6 arrive · 0 skip · 955 rows · finished · mapMatches

    event    bcn         od        sd    ts
    set      1       228.92    228.92     2
    arrive   1         4.02      4.02     4
    clear    1         4.02         —     0
    set      2        63.96     63.96     2     ← the switch, same tick
    ...
    clear    6         4.75         —     0     ← the terminal release

★★★ **The overwrite is instantaneous and needs no release.** `set` follows `clear` inside one
tick and the next engine distance reads the *new* beacon. Battlewrath's read was right — the
pin is a slot you write to, and "release → set" was negotiating a lock the client doesn't
have.

★★ **`ts = 0` at every clear is the answer to the one genuinely unproven question.** It is the
client confirming the **view** is gone rather than only the state changing. A route ending
armed would have left a ghost arrow that looked live — and it would have looked like working.

★★★ **The calibration pair survives the target moving: worst `|sd − od|` = 1.99e-05 over 955
rows across six pins.** That figure had only ever been measured against one pin held for a
whole capture. `od` tracks whatever the pin currently *is*, with no drift at the switch.

⚠ **Every arrival landed at 4.02–4.75 yd against a 5.0 trigger, never closer.** At 0.2 s and
walking pace the last yard is crossed between samples, so **a 5 yd trigger fires at ~4.0–4.8**.
That is W1.6's closed form appearing in live data rather than in a fixture — the same
phase-and-stride effect, arriving from the client.

⚠ And what this is **not**: `task_chain` matches on raw distance only. No band, no rejection,
no stacked-beacon case, no ratchet. It proves the pointing; it decides nothing hard.

## Open — reduced §288, W6 now closed
