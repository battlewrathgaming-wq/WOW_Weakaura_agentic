...
## THE CLOSED LISTS AND THE NUMBERS
    Routes.R_FLOOR    5    ⚠⚠ **DERIVED, NOT CHOSEN:** `R_min = v_ceiling × POLL_MIN / 2 = 100 × 0.1 / 2`.
                           ...
                           the others move. ★ L18's SETTLED PAIRING: the literal stays (greppable, no
    ...
    Bucket.BAND_DEFAULT 2.5  CHOSEN, with its reason: the measured jump apex is ~1.64 (four flat jumps
                           ...
                           correctly a literal carrying its reason, and explicitly not L18's case.
...
## WHERE IT IS RULED (read these; this page only points)
    driver_architecture.md        §4b (the characteristic record) · §5 L18 (load-bearing ⟹ sourceable)
    ...
    ARCHITECT_LOG.md              AL-28 (L19: a hedged answer with a physical reason is a SPEC FOR A
                                  MEASUREMENT — the band ceiling is one) · AL-26 (L18, the settled pairing)
...
climbs the ladder is not** (RI-64). ⬜ The L18 ASSERTION pairing `R_FLOOR` to `MAX_CLOSING_SPEED ×
POLL_MIN / 2` is the bench's one line and is not yet written. ⬜ The BAND'S UPPER LIMIT is undefined and
is a MEASUREMENT, not a preference (L19 — *floor above clipping* is a distance between two standable
