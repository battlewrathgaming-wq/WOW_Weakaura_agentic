...
| ★★★ **no validation on authoring** | refusing the author's input. Duplicate stages, out-of-order stages and fractions are all legal — they are **told** what they are doing (match count, gaps line, running order) and then trusted with it. ⚠ Refusing would be **grading the work** | `COA_DungeonRun/routes.lua:482` |
...
| ★ **baseline OFF** | shipping switched-on. Presence is not permission. ⚠ Wheel-zoom and right-drag default off because **the wheel belongs to the world camera and right-drag to camera-look** — an addon that takes either on install has taken something nobody offered | `COA_DungeonRun/map.lua:1874` |
