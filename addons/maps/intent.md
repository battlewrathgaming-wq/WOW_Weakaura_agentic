...
| pick which ART a point draws with | a **pure** resolver, tested · **ours** | ⚠ **A wrong art key still renders** — every wrong answer draws *something*, so there is no failing case to notice. That is why the resolver is pure and has its own fixtures rather than being trusted *(`COA_DungeonRun/map.lua:808`)* |
