...
| ★★★ **no validation on authoring** | refusing the author's input. Duplicate stages, out-of-order stages and fractions are all legal — they are **told** what they are doing (match count, gaps line, running order) and then trusted with it. ⚠ Refusing would be **grading the work** | `COA_DungeonRun/routes.lua:488` |
