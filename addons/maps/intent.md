...
| ★★★ **no validation on authoring** | refusing the author's input. Duplicate stages, out-of-order stages and fractions are all legal — they are **told** what they are doing (match count, gaps line, running order) and then trusted with it. ⚠ Refusing would be **grading the work** | `COA_DungeonRun/routes.lua:723` |
...
| ★ **baseline OFF** | shipping switched-on. Presence is not permission. ⚠ Wheel-zoom and right-drag default off because **the wheel belongs to the world camera and right-drag to camera-look** — an addon that takes either on install has taken something nobody offered | `COA_DungeonRun/map.lua:1895` |
...
| pick which ART a point draws with | a **pure** resolver, tested · **ours** | ⚠ **A wrong art key still renders** — every wrong answer draws *something*, so there is no failing case to notice. That is why the resolver is pure and has its own fixtures rather than being trusted *(`COA_DungeonRun/map.lua:880`)* |
