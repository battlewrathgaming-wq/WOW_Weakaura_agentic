...
| **SILENT** | ★★ | every beacon ICON ranks as a BEACON - `kill` is a WORD a beacon | `note` | `COA_DungeonRun/map.lua:195` |
| **CULTURE** | ★★ | wheel-zoom and right-drag default OFF - the wheel belongs to the | `wheelTick` | `COA_DungeonRun/map.lua:1745` |
...
| — | ★★★ | the PRECEDENCE LADDER is not a display preference - enter and terminal | `RANK` | `COA_DungeonRun/map.lua:141` |
| — | ★★★ | §61 - promoted objects sit ABOVE the pin. The authored thing outranks its | `RANK` | `COA_DungeonRun/map.lua:181` |
| — | ★★★ | §48 - TIME IS THE MAIN FILTER, and three quantities must not be conflated | `Map.TimeSpan` | `COA_DungeonRun/map.lua:534` |
| — | ★★★ | §43 - CURATION EDITS THE VIEW, NEVER THE CAPTURE | `Map.Hidden` | `COA_DungeonRun/map.lua:789` |
| — | ★★★ | §34 - SELECTION is the SINGLE coupling point between the two frames | `Map.SetMoveArmed` | `COA_DungeonRun/map.lua:855` |
| — | ★★★ | MANY LISTENERS, never one slot (§63) - §61 adds a third surface, and one | `Map.SetMoveArmed` | `COA_DungeonRun/map.lua:864` |
| — | ★★★ | it is an OBJECT, NOT A MODE - his: *"It is on that object. Only promoted | `Map.SetMoveArmed` | `COA_DungeonRun/map.lua:874` |
| — | ★★★ | the MAP answers "what is this?" - his, 2026-08-13. The map identifies; | `TIP_COLOR` | `COA_DungeonRun/map.lua:975` |
| — | ★★★ | DRAGGABLE MEANS PROMOTED - a node is CAPTURE, and DR-9 with §43 forbid | `Map.Draggable` | `COA_DungeonRun/map.lua:1089` |
| — | ★★★ | §36 - LOCATION SORTS THE LIST; IT NEVER CHOOSES THE VIEW | `Map.RunList` | `COA_DungeonRun/map.lua:1154` |
| — | ★★★ | §76 - the panel is UI, NOT map. His: *"zoom shouldn't mean the content | `local ax, ay = Map.ReadoutAnchor(dx * zo` | `COA_DungeonRun/map.lua:1440` |
...
| — | ★★ | DR-35 puts the COMBAT leg BELOW the travel leg - where two paths | `RANK` | `COA_DungeonRun/map.lua:169` |
| — | ★★ | DR-36 puts the PIN above the terminal stop - it is the only point that | `RANK` | `COA_DungeonRun/map.lua:175` |
| — | ★★ | markers SCALE WITH THE MAP (his call) - a marker represents a FOOTPRINT, | `local ZOOM_MIN, ZOOM_MAX = 1.0, 4.0` | `COA_DungeonRun/map.lua:232` |
| — | ★★ | `timed` is the flag that matters - a run's envelope is a coordinate in | `RUN_LISTS` | `COA_DungeonRun/map.lua:309` |
| — | ★★ | §60's SECOND PLANE cost exactly one row, which is what the table was | `{ key = "notes", timed = false, art = fa` | `COA_DungeonRun/map.lua:324` |
| — | ★★ | each slot resolves through its OWN store - the route side reads NS.Routes | `resolve` | `COA_DungeonRun/map.lua:341` |
| — | ★★ | match on the mapID the RUN'S OWN POINTS carry - display reads it from the | `Map.MapIDOf` | `COA_DungeonRun/map.lua:454` |
| — | ★★ | GUARDED ON IDENTITY, and the guard is the whole point | `Map.ArtFor` | `COA_DungeonRun/map.lua:493` |
| — | ★★ | WHICH LISTS a layer keeps is the LAYER'S to declare (§61) | `Map.PointsOn` | `COA_DungeonRun/map.lua:515` |
| — | ★★ | SKIP IS DERIVED, never another decision to make - window / 10, floored | `Map.SkipStep` | `COA_DungeonRun/map.lua:582` |
| — | ★★ | §48's LADDER, in order, each rung only NARROWING what the one above left | `Map.VisibleOn` | `COA_DungeonRun/map.lua:722` |
| — | ★★ | ONE PLACE draws everything, from every loaded layer, in layer order | `Map.Painted` | `COA_DungeonRun/map.lua:753` |
| — | ★★ | a BEACON DRAWS AS ITS ICON - the field the user picked, not a type we | `if point.kind == "beacon" then` | `COA_DungeonRun/map.lua:826` |
| — | ★★ | §34's boundary for the THIRD gesture - the map OWNS the right-click | `Map.AddOnEdit` | `COA_DungeonRun/map.lua:896` |
| — | ★★ | Describe is PURE, because it IS the whole readout - a wrong word here is | `Map.Describe` | `COA_DungeonRun/map.lua:945` |
| — | ★★ | CLAMPED, because off the art is not a position | `Map.FractionAt` | `COA_DungeonRun/map.lua:1074` |
| — | ★★ | a ROUTE ALONE is a legitimate view - §61's none-option on the run slot | `if routeName then` | `COA_DungeonRun/map.lua:1215` |
| — | ★★ | §69 - THREE GESTURES on one object: hover reads, left click selects | `d:RegisterForClicks` | `COA_DungeonRun/map.lua:1248` |
| — | ★★ | the OnUpdate exists ONLY while the drag is in flight - installed on | `d:RegisterForDrag` | `COA_DungeonRun/map.lua:1257` |
| — | ★★ | §20.2 - LOCATION SEEDS THE VIEW, read from the SAME calls capture used | `context` | `COA_DungeonRun/map.lua:1357` |
| — | ★★ | ONE CONTENT SOURCE, TWO PRESENTATIONS - both render Map.Describe, so the | `READOUT_ROWS` | `COA_DungeonRun/map.lua:1375` |
| — | ★★ | the NOTE PLANE is LOAD-DRIVEN like every other layer - a plane that | `loaded.notes` | `COA_DungeonRun/map.lua:1524` |
| — | ★★ | NO ARGUMENT = NO RUN (§36) - the retirement of the auto-pick. Guessing | `Map.Show` | `COA_DungeonRun/map.lua:1557` |
| — | ★★ | ARMED, AND THIS EXACT OBJECT - one guard, not two. A second Draggable | `if not dot or not armed or dot.point ~` | `COA_DungeonRun/map.lua:1609` |
| — | ★★ | A CLICK DROPS IT - what he reached for, rather than the client's own | `canvas:EnableMouse` | `COA_DungeonRun/map.lua:1629` |
| — | ★★ | the button READS THE VIEW, never a stored index - it shows where you | `refreshControls` | `COA_DungeonRun/map.lua:1683` |
| — | ★★ | a pane that REMOVES an object asks for a redraw, and must not have to know | `applyView` | `COA_DungeonRun/map.lua:1802` |
| — | ★★ | §76 - THE ZOOM CONTROLS LIVE ON THE MAP, not in curation | `applyView` | `COA_DungeonRun/map.lua:1808` |
| — | ★★ | THE NEXT STAGE ABOVE WHERE YOU ARE, wrapping at the top - defined against | `Map.NextStage` | `COA_DungeonRun/map.lua:1856` |
| — | ★★ | THE COMMAND STRIP is his layout - one header row, and the bottom bar is | `local MARGIN, STRIP, FOOT = 16, 40, 14` | `COA_DungeonRun/map.lua:1968` |
| — | ★★ | HIGH, above the action bars - his: *"when you're using it, you're using | `frame:SetFrameStrata` | `COA_DungeonRun/map.lua:1987` |
...
| **SILENT** | ★★★ | WorldMapDetailFrame is 1002x668 (coordinates) while the tile art is 4x3x256 = 1024x768 | `Map.Offset` | `COA_DungeonRun/map.lua:1056` |
| **SILENT** | ★★★ | a TERRAIN MAP shifts the dungeon level by ONE - the client's own | `Map.TilePath` | `COA_DungeonRun/map.lua:1100` |
...
| **SILENT** | ★★ | frame level drives HIT TESTING as well as draw order, and ties | `RANK` | `COA_DungeonRun/map.lua:161` |
| **SILENT** | ★★ | GetCurrentMapAreaID() is OFF BY ONE from the internal mapID - the | `Map.MapIDOf` | `COA_DungeonRun/map.lua:460` |
| **SILENT** | ★★ | FLOOR INDEX IS NOT ROUTE ORDER - SFK_Run4 runs 1, 2, back to 1, 7, | `Map.FloorAt` | `COA_DungeonRun/map.lua:627` |
| **SILENT** | ★★ | a wrong ART KEY still renders - every wrong answer draws something, | `Map.ArtKey` | `COA_DungeonRun/map.lua:808` |
| **SILENT** | ★★ | written as a BRANCH, never `run and Map.ArtFor(...) or hereFile` | `if run then mapFile = Map.ArtFor(run, he` | `COA_DungeonRun/map.lua:1303` |
...
| — | ★★ | 3.3.5 has NO SetClipsChildren - a ScrollFrame viewport with the canvas as its | `local ZOOM_MIN, ZOOM_MAX = 1.0, 4.0` | `COA_DungeonRun/map.lua:226` |
| — | ★★ | ONE SECOND IS THE FLOOR - points carry `t` at second resolution, so a | `MIN_WIDTH` | `COA_DungeonRun/map.lua:565` |
...
| File | ★ | ★★ | ★★★ | ⚠ |
|---|---|---|---|---|
| `COA_DungeonRun/map.lua` | 1 | 68 | 15 | 12 |
